...
function RsvpPage({ params }: { params: { token: string } }) {
  ...
  return <main data-testid="rsvp-page" data-rsvp-state={status} className="min-h-[100dvh] bg-[var(--agency-paper)] p-5 pb-14 pt-8 text-[var(--agency-ink)] md:p-10 md:pb-16"><div className="mx-auto max-w-5xl">
    ...
    <section id="rsvp" className="scroll-mt-5"><form data-testid="rsvp-form" className={cn(CARD, "space-y-5 p-7")} onSubmit={async event => {
     ...
     try {
       ...
       <div><p className={EYEBROW}>RSVP</p><h2 className={cn(TITLE, "mt-3 text-2xl")}>{t('rsvp.form.title')}</h2><p className="mt-2 text-xs font-light leading-relaxed text-foreground/45">{t('rsvp.form.sub')}</p></div>
     ...
     {state.status === 'confirmed' && <div className="grid grid-cols-2 gap-3 text-sm">{(['ceremony', 'cocktail', 'dinner', 'brunch'] as const).map(key => <label key={key} className="flex gap-2"><input aria-label={attendLabels[key]} name={`rsvp-${key}`} type="checkbox" checked={state[key]} disabled={status !== 'ready'} onChange={e => setState(s => ({ ...s, [key]: e.target.checked }))} className="accent-[var(--agency-ink)]" />{attendLabels[key]}</label>)}<label className="flex gap-2 col-span-2"><input aria-label="plus-one" name="rsvp-plus-one" type="checkbox" checked={state.plusOne} disabled={status !== 'ready'} onChange={e => setState(s => ({ ...s, plusOne: e.target.checked }))} className="accent-[var(--agency-ink)]" />{t('rsvp.form.plusOne')}</label></div>}
     <input aria-label="dietary" name="rsvp-dietary" className={FIELD} placeholder={t('rsvp.form.dietary')} value={state.dietary} disabled={status !== 'ready'} onChange={e => setState(s => ({ ...s, dietary: e.target.value }))} />
     <textarea aria-label="notes" name="rsvp-notes" className={FIELD} placeholder={t('rsvp.form.notes')} value={state.notes} disabled={status !== 'ready'} onChange={e => setState(s => ({ ...s, notes: e.target.value }))} />
    ...
    <section id="jour-j" className={cn(CARD, "scroll-mt-5 p-7")}><p className={EYEBROW}>{t('rsvp.nav.day').toUpperCase()}</p><h2 className={cn(TITLE, "mt-3 text-2xl")}>{t('rsvp.day.title')}</h2>
      {status === 'loading' ? <p data-testid="status-program-loading" className={cn(BODY, "mt-4 text-sm")} role="status">{t('rsvp.loading')}</p> : <>{portal.guest?.tableName && <p data-testid="text-table-name" className="mt-4 rounded-xl bg-foreground/5 p-3 text-sm">{t('rsvp.table')} <strong>{portal.guest.tableName}</strong></p>}
      {portal.program?.length ? <ol className="mt-4 space-y-3">{portal.program.map(item => <li data-testid={`card-program-${item.id}`} key={item.id} className="border-l border-foreground/25 pl-3"><p className="text-xs text-[var(--agency-body)]">{formatPortalTime(item.time, true)}{item.endTime ? ` — ${formatPortalTime(item.endTime, true)}` : item.durationMinutes ? ` · ${item.durationMinutes} ${t('rsvp.minutes')}` : ''}</p><h3>{item.title}</h3>{item.detail && <p className="text-sm text-foreground/60">{item.detail}</p>}{item.location && <p className="text-xs text-[var(--agency-body)]">{item.location}</p>}</li>)}</ol> : <p data-testid="status-program-empty" className={cn(BODY, "mt-4 text-sm")}>{t('rsvp.programEmpty')}</p>}
    ...
    <section id="partager" className={cn(CARD, "scroll-mt-5 p-7")}><p className={EYEBROW}>{t('rsvp.share.eyebrow')}</p><h2 className={cn(TITLE, "mt-3 text-2xl")}>{t('rsvp.share.title')}</h2>
      {!policy?.enabled ? <p data-testid="status-media-disabled" className={cn(BODY, "mt-4 text-sm")}>{t('rsvp.share.disabled')}</p> : <form className="mt-4 space-y-3" onSubmit={async event => { event.preventDefault(); const form = event.currentTarget; const file = (form.elements.namedItem('media-file') as HTMLInputElement).files?.[0]; if (!file) return setMediaError(t('rsvp.share.chooseFile')); if (policy.maxSize && file.size > policy.maxSize) return setMediaError(t('rsvp.share.tooBig', { size: Math.round(policy.maxSize / 1024 / 1024) })); setMediaPending(true); setMediaError(''); try { const request = await fetch(`/api/rsvp/${params.token}/media/uploads/request-url`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: file.name, contentType: file.type, size: file.size }) }); const upload = await request.json().catch(() => ({})); if (!request.ok || !upload.uploadURL) throw new Error(upload.error || t('rsvp.share.prepareError')); const put = await fetch(upload.uploadURL, { method: 'PUT', headers: { "Content-Type": file.type }, body: file }); if (!put.ok) throw new Error(t('rsvp.share.uploadError')); const saved = await fetch(`/api/rsvp/${params.token}/media`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ objectPath: upload.objectPath, finalizeToken: upload.finalizeToken, name: file.name, contentType: file.type, size: file.size, caption: caption || undefined, visibility, consent: true }) }); if (!saved.ok) { const body = await saved.json().catch(() => ({})); throw new Error(body.error || t('rsvp.share.publishError')); } setCaption(''); form.reset(); await loadPortal(); } catch (reason) { setMediaError(reason instanceof Error ? reason.message : t('rsvp.share.publishError')); } finally { setMediaPending(false); } }}>
        <input data-testid="input-media-file" name="media-file" type="file" accept={policy.accept?.join(',')} disabled={mediaPending} className="block w-full text-sm" /><input data-testid="input-media-caption" value={caption} onChange={e => setCaption(e.target.value)} placeholder={t('rsvp.share.caption')} disabled={mediaPending} className={FIELD} /><select data-testid="select-media-visibility" value={visibility} onChange={e => setVisibility(e.target.value as 'couple' | 'guests')} disabled={mediaPending} className={FIELD}><option value="couple">{t('rsvp.share.forCouple')}</option><option value="guests">{t('rsvp.share.forGuests')}</option></select>{mediaError && <p data-testid="media-error" className="text-sm text-destructive" role="alert">{mediaError}</p>}<button data-testid="button-media-upload" disabled={mediaPending} className={PILL_INK}>{mediaPending ? t('rsvp.share.sending') : t('rsvp.share.upload')}</button></form>}
      {portal.contributions?.length ? <ul className="mt-5 space-y-2">{portal.contributions.map(item => <li data-testid={`media-${item.id}`} key={item.id} className="flex items-center justify-between gap-3 text-sm"><span>{item.caption || item.name} <em className="text-[var(--agency-body)]">· {item.moderationStatus}</em></span>{item.canView ? <a data-testid={`link-media-${item.id}`} className="underline" href={`/api/rsvp/${params.token}/media/${item.id}`}>{t('rsvp.share.view')}</a> : <span className="text-foreground/45">{t('rsvp.share.pending')}</span>}</li>)}</ul> : <p data-testid="status-media-empty" className={cn(BODY, "mt-5 text-sm")}>{t('rsvp.share.empty')}</p>}</section>
    <section id="musique" className={cn(CARD, "scroll-mt-5 p-7")}><p className={EYEBROW}>{t('rsvp.music.eyebrow')}</p><h2 className={cn(TITLE, "mt-3 text-2xl")}>{t('rsvp.music.title')}</h2><form className="mt-4 space-y-3" onSubmit={async event => { event.preventDefault(); setSongError(''); setSongPending(true); try { const response = await fetch(`/api/rsvp/${params.token}/song-requests`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(song) }); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.error || t('rsvp.music.error')); setSong({ title: '', artist: '', message: '' }); await loadPortal(); } catch (reason) { setSongError(reason instanceof Error ? reason.message : t('rsvp.music.error')); } finally { setSongPending(false); } }}><input data-testid="input-song-title" required value={song.title} onChange={e => setSong(s => ({ ...s, title: e.target.value }))} placeholder={t('rsvp.music.titleField')} className={FIELD} /><input data-testid="input-song-artist" required value={song.artist} onChange={e => setSong(s => ({ ...s, artist: e.target.value }))} placeholder={t('rsvp.music.artist')} className={FIELD} /><input data-testid="input-song-message" value={song.message} onChange={e => setSong(s => ({ ...s, message: e.target.value }))} placeholder={t('rsvp.music.note')} className={FIELD} />{songError && <p data-testid="song-error" role="alert" className="text-sm text-destructive">{songError}</p>}<button data-testid="button-song-submit" disabled={songPending} className={PILL_INK}>{songPending ? t('rsvp.music.sending') : t('rsvp.music.submit')}</button></form>{portal.songRequests?.length ? <ul className="mt-5 space-y-2">{portal.songRequests.map(item => <li data-testid={`song-${item.id}`} key={item.id} className="text-sm">{item.title} — {item.artist} <span className="text-[var(--agency-body)]">· {item.status}</span></li>)}</ul> : <p data-testid="status-songs-empty" className={cn(BODY, "mt-5 text-sm")}>{t('rsvp.music.empty')}</p>}</section>
    <section id="apres" className="scroll-mt-5 rounded-3xl border border-border bg-card p-7 md:col-span-2"><p className={EYEBROW}>{t('rsvp.after.eyebrow')}</p><h2 className={cn(TITLE, "mt-3 text-2xl")}>{t('rsvp.after.title')}</h2>{portal.afterContent?.length ? <ul className="mt-4 grid gap-3 md:grid-cols-2">{portal.afterContent.map(item => <li data-testid={`after-${item.id}`} key={item.id} className="rounded-xl bg-foreground/5 p-4"><p className="text-xs text-[var(--agency-body)]">{formatPortalTime(item.time)}{item.location ? ` · ${item.location}` : ''}</p><h3>{item.title}</h3>{item.detail && <p className="mt-1 text-sm text-foreground/60">{item.detail}</p>}</li>)}</ul> : <p data-testid="status-after-empty" className={cn(BODY, "mt-4 text-sm")}>{t('rsvp.after.empty')}</p>}</section>
    </div>
    {/*
      Une URL d'invité est publique : elle porte l'identité de l'agence et ses
      textes légaux, comme le reste du site. Pas de barre de navigation —
      l'invité n'a pas à se promener, il a une réponse à donner.
    */}
    <SiteFooter />
  </div></main>;
}
...
function Routes() {
  return <RoutedErrorBoundary><Suspense fallback={<RouteFallback />}><Switch>
    ...
    <Route path="/dossiers"><Redirect to="/user-portal" /></Route>
