...
export function NetworkPage() {
    ...
    return (
      <main className="grid min-h-[100dvh] place-items-center bg-background text-foreground">
        <div className="text-center" role="status">
          <p className="text-[10px] uppercase tracking-[.28em] text-foreground/40">Réseau AIME</p>
          <p className="mt-3 text-sm text-foreground/70">Ouverture du Monde…</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <div data-testid="network-page" className="flex h-[100dvh] w-full overflow-hidden bg-background text-foreground selection:bg-foreground/20 font-sans">
      ...
      <div className="relative z-10 flex h-full w-full pointer-events-none">
        ...
        {/* Mobile View Toggle */}
        <div className="md:hidden absolute top-6 left-1/2 -translate-x-1/2 z-50 pointer-events-auto flex items-center rounded-full border border-foreground/10 bg-background/60 backdrop-blur-xl p-1 shadow-xl">
           <button aria-pressed={mobileView === 'list'} onClick={() => setMobileView('list')} className={cn("px-4 py-1.5 rounded-full text-[11px] font-medium transition-colors", mobileView === 'list' ? "bg-foreground text-background" : "text-foreground/60 hover:text-foreground")}>
             Liste
           </button>
           <button aria-pressed={mobileView === 'grid'} onClick={() => setMobileView('grid')} className={cn("px-4 py-1.5 rounded-full text-[11px] font-medium transition-colors", mobileView === 'grid' ? "bg-foreground text-background" : "text-foreground/60 hover:text-foreground")}>
             Grille
           </button>
        </div>
        ...
        {/* Sidebar */}
        <div className={cn(
          "pointer-events-auto absolute inset-y-0 left-0 z-40 flex w-full max-w-[400px] flex-col border-r border-foreground/10 bg-background/90 md:bg-background/80 backdrop-blur-2xl transition-transform duration-500 cubic-bezier(0.16, 1, 0.3, 1) md:relative md:translate-x-0",
          ...
          <div className="flex items-center justify-between border-b border-foreground/10 px-6 py-5 mt-14 md:mt-0">
           ...
           <div className="flex-1 overflow-y-auto p-6 space-y-10 hide-scrollbar" role="listbox" aria-label="Personnes du Monde">
             ...
             <div className="space-y-4">
               <h2 className="text-[10px] font-semibold tracking-widest text-foreground/40 uppercase">Dans le réseau</h2>
               <div className="space-y-2">
                  ...
                  {people.length === 0 && <p className="text-[13px] text-foreground/30 italic">Aucune personne trouvée dans le réseau</p>}
               </div>
             </div>
          </div>
        </div>
      ...
      {activePerson && (
        <CenteredBlock
          ...
          leading={<span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-foreground text-background"><Users className="h-5 w-5" /></span>}
        >
          <div className="flex flex-col gap-5">
            <div className="flex flex-wrap gap-2 text-[11px] font-medium">
               {activePerson.status && (
                 <span className="flex items-center gap-1.5 rounded-full border border-foreground/10 bg-foreground/5 px-3 py-1.5 text-foreground/70">
            ...
            <div className="mt-1 flex gap-2">
              {canEdit && activePerson.contact && (
                <a
                  ...
                  className="flex-1 flex items-center justify-center gap-2 rounded-xl border border-foreground/15 bg-foreground/5 px-4 py-3 text-xs font-semibold text-foreground hover:bg-foreground/10 transition-colors"
                >
                  <ArrowUpRight className="h-3.5 w-3.5" /> Contacter
                </a>
              )}
              <Link href={activePerson.type === 'me' ? '/profile' : '/user-portal'} className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-foreground text-background px-4 py-3 text-xs font-semibold hover:bg-foreground/90 transition-colors">
  ...
  return (
    ...
    >
       <div className={cn(
         ...
         isActive ? "bg-foreground text-background border-border" : "bg-foreground/5 border-foreground/10 text-foreground/70"
       )}>
         <Users className="h-4 w-4" />
       </div>
       <div className="flex-1 min-w-0">
         <div className={cn("truncate text-[13px] font-medium transition-colors", isActive ? "text-foreground" : "text-foreground/90")}>{person.name}</div>
         <div className="truncate text-xs text-foreground/50">{person.role}</div>
       </div>
    </button>
  );
}
