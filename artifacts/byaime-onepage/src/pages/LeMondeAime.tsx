...
import { DIMENSIONS, type WorldDimension, type WorldEvent } from "@/lib/world-model/types";
...
type TemporalScope = "maintenant" | "aujourdhui" | "semaine" | "annee" | "histoire";
const TEMPORAL_LABELS: Record<TemporalScope, string> = {
...
export function LeMondeAimePage() {
  const [activeDimensions, setActiveDimensions] = useState<Set<WorldDimension>>(new Set(DIMENSIONS));
  const [selectedEventId, setSelectedEventId] = useState<string | null>(null);
  
  // Realtime & Scopes
  const [currentTime, setCurrentTime] = useState<number>(Date.now());
  const [isPlaying, setIsPlaying] = useState(true);
  const [spatialScope, setSpatialScope] = useState<string>("monde"); 
  const [temporalScope, setTemporalScope] = useState<TemporalScope>("histoire");

  // Timeline Zoom Window (in ms)
  const [timelineSpan, setTimelineSpan] = useState<number>(86400000); // 1 day default window
  ...
  const [historicalEvents, setHistoricalEvents] = useState<WorldEvent[]>([]);
  const [liveEvents, setLiveEvents] = useState<WorldEvent[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const allEvents = useMemo(
    () => [...historicalEvents, ...liveEvents].sort((a, b) => a.timestamp - b.timestamp),
  ...
  const providerFilter = useMemo(() => {
    const filter: {
      ...
      country?: string;
      location?: string;
    } = { dimension: [...activeDimensions] };

    if (CONTINENTS.includes(spatialScope)) filter.continent = spatialScope;
    else if (COUNTRIES.includes(spatialScope)) filter.country = spatialScope;
    else if (spatialScope !== "monde") filter.location = spatialScope;
  ...
  const synchronisations = useMemo(() => {
    const clusters: { category: string; events: WorldEvent[] }[] = [];
  ...
  const currentSummary = useMemo(() => {
    const active = filteredEvents.filter(event =>
      event.timestamp <= currentTime && event.timestamp >= currentTime - 86400000
    );
  ...
  const toggleDimension = (dim: WorldDimension) => {
  ...
  return (
    <main className="fixed inset-0 overflow-hidden bg-[#020202] text-white flex flex-col font-sans">
      ...
      <AnimatePresence>
        {selectedEvent && (
          <CenteredBlock 
            eyebrow="Événement mondial simulé" 
        ...
        {searchOpen && (
          <CenteredBlock eyebrow="Recherche" title="Explorer la partition" onClose={() => setSearchOpen(false)}>
            <input 
              autoFocus
              type="text" 
