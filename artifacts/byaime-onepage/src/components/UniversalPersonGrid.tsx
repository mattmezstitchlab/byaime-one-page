...
import type { PersonNode } from '@/lib/universal/people-grid';

const GRID_SIZE = 280;

function squareRingPosition(index: number, startRing: number) {
  let ring = startRing;
  let offset = index;
  while (offset >= ring * 8) {
    offset -= ring * 8;
  ...
  if (offset < sideLength) return { x: -ring + offset, y: -ring, ring };
  if (offset < sideLength * 2) return { x: ring, y: -ring + (offset - sideLength), ring };
  if (offset < sideLength * 3) return { x: ring - (offset - sideLength * 2), y: ring, ring };
  return { x: -ring, y: ring - (offset - sideLength * 3), ring };
}

function nextRingAfter(count: number, startRing: number) {
  let ring = startRing;
  let remaining = count;
...
export function UniversalPersonGrid({ 
  people,
  activeId,
  allowContact = false,
  onSelect,
  ...
  const { nodes, edges } = useMemo(() => {
    const resultNodes: GridNode[] = [];
    const resultEdges: GridEdge[] = [];
  ...
  const handleKeyboardNavigation = (event: React.KeyboardEvent<HTMLDivElement>) => {
    const panStep = 64;
    if (event.key === 'ArrowLeft') x.set(x.get() + panStep);
    else if (event.key === 'ArrowRight') x.set(x.get() - panStep);
    else if (event.key === 'ArrowUp') y.set(y.get() + panStep);
    else if (event.key === 'ArrowDown') y.set(y.get() - panStep);
    else if (event.key === '+' || event.key === '=') scale.set(Math.min(3, scale.get() + 0.2));
    else if (event.key === '-') scale.set(Math.max(0.15, scale.get() - 0.2));
    else if (event.key === 'Home') resetView();
    else if (event.key === 'PageUp') setDepth((current) => Math.max(0, current - 1));
    else if (event.key === 'PageDown') setDepth((current) => Math.min(2, current + 1));
    else return;
          ...
          return (
            ...
            >
              {node.isCenter ? (
                <button
                  type="button"
                  className="group relative flex flex-col items-center rounded-full focus:outline-none focus-visible:ring-2 focus-visible:ring-white"
      ...
      <div className="absolute right-3 top-3 z-10 flex items-center gap-1 rounded-full border border-foreground/10 bg-background/80 p-1 backdrop-blur-xl sm:right-6 sm:top-6">
        {[
          ...
          <button
            key={layer.value}
            type="button"
            aria-label={`Afficher jusqu’à la profondeur ${layer.label} · ${layer.title}`}
            aria-pressed={depth === layer.value}
