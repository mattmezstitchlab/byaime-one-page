...
export function ProfileNervousSystem() {
  ...
  return (
    <div className="aime-cinematic-surface absolute inset-0 h-full w-full overflow-hidden rounded-3xl border border-foreground/5 bg-[#020202] shadow-2xl">
      ...
      <div className="absolute inset-0 pointer-events-none">
          {nodes.map(node => {
            ...
            return (
              ...
              >
                {isIdentity && (
                  <div className="w-24 h-24 rounded-full border border-foreground/20 bg-foreground/5 backdrop-blur-md flex items-center justify-center shadow-[0_0_30px_rgba(255,255,255,0.1)] group-hover:bg-foreground/10 transition-colors">
                    <User className="w-10 h-10 text-foreground/80" />
                  </div>
                )}
                
                {isRoot && (
                  <div className="w-14 h-14 rounded-full border border-foreground/10 bg-[#0a0a0a] flex items-center justify-center shadow-lg group-hover:border-foreground/30 transition-colors">
                    {node.icon && <node.icon className="w-6 h-6 text-foreground/50 group-hover:text-foreground transition-colors" />}
                  </div>
                )}
                
                {node.type === "item" && (
                  <div className="w-3 h-3 rounded-full bg-foreground/20 group-hover:bg-white group-hover:scale-150 transition-all shadow-[0_0_10px_rgba(255,255,255,0.2)]" />
                )}
                
                <span className={cn(
                  "absolute whitespace-nowrap text-center transition-all",
                  isIdentity ? "top-full mt-4 text-sm font-display tracking-widest uppercase text-foreground" :
                  isRoot ? "top-full mt-3 text-[10px] tracking-widest uppercase text-foreground/60 group-hover:text-foreground" :
                  "opacity-0 group-hover:opacity-100 top-full mt-2 text-[9px] uppercase tracking-wider text-foreground/80 bg-background/60 px-2 py-1 rounded backdrop-blur-md border border-foreground/10"
                )}>
                  {node.label}
                </span>
              </motion.button>
...
function EditorModal({ node, nodes, setSelectedNode, onClose, project, updateProject, updateEntity, openUserProfile, currentRole }: any) {
      ...
      <CenteredBlock eyebrow="Édition" title="Le Monde" onClose={onClose} leading={<Globe className="mt-4 w-6 h-6 text-foreground/50" />}>
         <div className="space-y-6 mt-4">
            <div>
               ...
               <input 
                  type="text" 
                  className="w-full bg-foreground/5 border border-foreground/10 rounded-xl px-4 py-3 text-sm text-foreground focus:outline-none focus:border-foreground/30 transition-colors"
       ...
       <CenteredBlock eyebrow="Édition du Moment" title={event.title} onClose={onClose} leading={<Calendar className="mt-4 w-6 h-6 text-foreground/50" />}>
         <div className="space-y-6 mt-4">
            ...
            <div>
               ...
               <textarea 
                  className="w-full bg-foreground/5 border border-foreground/10 rounded-xl px-4 py-3 text-sm text-foreground focus:outline-none focus:border-foreground/30 transition-colors min-h-[100px] resize-none"
           ...
           <div className="space-y-6 mt-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div>
                    ...
                    <select 
                       className="w-full bg-[#0a0a0a] border border-foreground/10 rounded-xl px-4 py-3 text-sm text-foreground focus:outline-none focus:border-foreground/30 transition-colors appearance-none"
        ...
        <CenteredBlock eyebrow="Édition Finance" title={payment.label} onClose={onClose} leading={<Folder className="mt-4 w-6 h-6 text-foreground/50" />}>
           <div className="space-y-6 mt-4">
              <div>
                 ...
                 <input type="text" className="w-full rounded-xl border border-foreground/10 bg-foreground/5 px-4 py-3 text-sm text-foreground outline-none transition-colors focus:border-foreground/30" defaultValue={payment.label} onBlur={(e) => updateEntity("payments", payment.id, { label: e.target.value })} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                 <div>
                    <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Montant</label>
                    <input type="number" min="0" step="0.01" className="w-full rounded-xl border border-foreground/10 bg-foreground/5 px-4 py-3 text-sm text-foreground outline-none transition-colors focus:border-foreground/30" defaultValue={payment.amountCents / 100} onBlur={(e) => updateEntity("payments", payment.id, { amountCents: Math.round(Number(e.target.value || 0) * 100) })} />
                 </div>
                 <div>
                    <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">État</label>
                    <select className="w-full appearance-none rounded-xl border border-foreground/10 bg-[#0a0a0a] px-4 py-3 text-sm text-foreground outline-none transition-colors focus:border-foreground/30" defaultValue={payment.state} onChange={(e) => updateEntity("payments", payment.id, { state: e.target.value })}>
              ...
              <div>
                 <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Date</label>
                 <input type="date" className="w-full rounded-xl border border-foreground/10 bg-foreground/5 px-4 py-3 text-sm text-foreground outline-none transition-colors focus:border-foreground/30" defaultValue={payment.at ? format(payment.at, "yyyy-MM-dd") : ""} onChange={(e) => e.target.value && updateEntity("payments", payment.id, { at: new Date(e.target.value).getTime() })} />
              </div>
           </div>
        </CenteredBlock>
     );
        ...
        <CenteredBlock eyebrow="Édition Réseau" title={guest.name} onClose={onClose} leading={<Network className="mt-4 w-6 h-6 text-foreground/50" />}>
           <div className="space-y-6 mt-4">
              ...
              <div>
                 ...
                 <textarea 
                    className="w-full bg-foreground/5 border border-foreground/10 rounded-xl px-4 py-3 text-sm text-foreground focus:outline-none focus:border-foreground/30 transition-colors min-h-[80px] resize-none"
     ...
     return (
        <CenteredBlock eyebrow="Édition Média" title={memory.title} onClose={onClose} leading={<ImageIcon className="mt-4 w-6 h-6 text-foreground/50" />}>
           <div className="space-y-6 mt-4">
              ...
              <div>
                 <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Statut</label>
                 <select 
                    className="w-full bg-[#0a0a0a] border border-foreground/10 rounded-xl px-4 py-3 text-sm text-foreground focus:outline-none focus:border-foreground/30 transition-colors appearance-none"
  ...
  if (node.type === "root") {
     ...
     return (
        <CenteredBlock eyebrow="Aperçu de la Branche" title={node.label} onClose={onClose} leading={<Link2 className="mt-4 w-6 h-6 text-foreground/50" />}>
           <div className="mt-6">
              {children.length > 0 ? (
                 <div className="grid gap-2 sm:grid-cols-2">
                    {children.map((child: NodeData) => (
                       <button 
                          ...
                          className="flex items-center gap-3 p-4 rounded-xl border border-foreground/5 bg-foreground/[0.02] hover:bg-foreground/[0.06] hover:border-foreground/20 transition text-left"
                       >
                          <div className="w-1.5 h-1.5 rounded-full bg-foreground/30 shrink-0" />
                          <span className="text-sm text-foreground/90 truncate">{child.label}</span>
                       </button>
                    ))}
                 </div>
              ) : (
                 <div className="text-center py-12 border border-dashed border-foreground/10 rounded-2xl bg-foreground/[0.02]">
                    <p className="text-foreground/40 text-sm">Aucun élément relié à cette branche pour le moment.</p>
                 </div>
              )}
           </div>
        </CenteredBlock>
