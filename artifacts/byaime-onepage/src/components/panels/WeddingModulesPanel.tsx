...
function Empty({ children }: { children: string }) {
  return <div className="rounded-2xl border border-dashed border-white/10 px-5 py-10 text-center text-sm text-white/40">{children}</div>;
}
...
export function WeddingModulesPanel({ module }: { module: WeddingModule }) {
  ...
  if (module === "seating") {
    ...
    return <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between"><div><p className="text-sm text-white/50">{unassigned.length} invité{unassigned.length > 1 ? "s" : ""} sans table</p></div><AddBar label="Ajouter une table" onAdd={() => addEntity("tables", { name: `Table ${project.tables.length + 1}`, capacity: 8 })} /></div>
      ...
      <div className="grid gap-3 md:grid-cols-2">{project.tables.map(table => { const guests = project.guests.filter(g => g.tableId === table.id); return <div key={table.id} className="rounded-2xl border border-white/10 bg-white/[.035] p-4"><div className="flex items-center justify-between"><div><h4 className="text-sm font-medium">{table.name}</h4><p className={cn("text-xs mt-1", guests.length > table.capacity ? "text-rose-300" : "text-white/40")}>{guests.length} / {table.capacity} places</p></div><button onClick={() => { guests.forEach(g => updateEntity("guests", g.id, { tableId: undefined })); removeEntity("tables", table.id); }} className="text-white/30 hover:text-rose-300"><Trash2 className="w-4 h-4" /></button></div><div className="mt-4 space-y-2">{guests.length === 0 ? <p className="text-xs text-white/30">Aucun invité assigné</p> : guests.map(g => <GuestSeat key={g.id} guest={g} tables={project.tables} onChange={tableId => updateEntity("guests", g.id, { tableId: tableId || undefined })} />)}</div></div> })}</div>
    </div>;
  }
  ...
  if (module === "ceremony") {
    ...
    return <div className="max-w-3xl mx-auto space-y-5"><EditableArea label="Intention et notes de cérémonie" value={c.notes} onChange={notes => updateProject({ ceremony: { ...c, notes } })} /><div className="grid gap-3 sm:grid-cols-2"><EditableArea label="Menu" value={c.menu} onChange={menu => updateProject({ ceremony: { ...c, menu } })} /><EditableArea label="Boissons" value={c.drinks} onChange={drinks => updateProject({ ceremony: { ...c, drinks } })} /><EditableArea label="Gâteau" value={c.cake} onChange={cake => updateProject({ ceremony: { ...c, cake } })} /><EditableArea label="Première danse" value={c.firstDance} onChange={firstDance => updateProject({ ceremony: { ...c, firstDance } })} /></div><div className="rounded-2xl border border-white/10 p-4"><p className="text-[10px] uppercase tracking-widest text-white/40 mb-3">Structure</p>{c.structure.map((item, i) => <div key={`${item}-${i}`} className="flex gap-3 py-2 border-b border-white/5 last:border-0 text-sm"><span className="text-white/30 font-mono">{String(i + 1).padStart(2, "0")}</span>{item}</div>)}</div><div className="rounded-2xl border border-white/10 p-4"><p className="text-[10px] uppercase tracking-widest text-white/40 mb-3">Lectures et vœux</p>{c.readings.map(r => <div key={r.id} className="mb-3"><p className="text-sm">{r.title} <span className="text-white/40">· {r.reader}</span></p><p className="text-xs text-white/45 mt-1">{r.text}</p></div>)}{c.vows.map(v => <EditableArea key={v.id} label={`Vœux de ${v.person}`} value={v.text} onChange={text => updateProject({ ceremony: { ...c, vows: c.vows.map(x => x.id === v.id ? { ...x, text } : x) } })} />)}</div></div>;
  }

  if (module === "music") return <CollectionPanel title="Bande-son du mariage" addLabel="Ajouter un morceau" onAdd={() => addEntity("music", { moment: "Nouveau moment", title: "À choisir", artist: "", status: "a_choisir" })}>{project.music.length === 0 ? <Empty>Aucun morceau associé à un moment.</Empty> : project.music.map(track => <div key={track.id} className="rounded-2xl border border-white/10 bg-white/[.035] p-4 flex gap-4 items-center"><button onClick={() => updateEntity("music", track.id, { status: track.status === "valide" ? "a_choisir" : "valide" })} className={cn("w-7 h-7 rounded-full border flex items-center justify-center", track.status === "valide" ? "border-emerald-400 text-emerald-300" : "border-white/20 text-white/30")}>{track.status === "valide" && <Check className="w-3.5 h-3.5" />}</button><div className="grid flex-1 gap-2 sm:grid-cols-3"><input value={track.moment} onChange={e => updateEntity("music", track.id, { moment: e.target.value })} className="bg-transparent text-xs text-white/50 outline-none" /><input value={track.title} onChange={e => updateEntity("music", track.id, { title: e.target.value })} className="bg-transparent text-sm outline-none" /><input value={track.artist} onChange={e => updateEntity("music", track.id, { artist: e.target.value })} placeholder="Artiste" className="bg-transparent text-sm text-white/60 outline-none placeholder:text-white/25" /></div><button onClick={() => removeEntity("music", track.id)} className="text-white/30 hover:text-rose-300"><Trash2 className="w-4 h-4" /></button></div>)}</CollectionPanel>;

  if (module === "logistics") {
    const l = project.logistics;
    return <div className="max-w-4xl mx-auto space-y-4"><EditableArea label="Parking" value={l.parking} onChange={parking => updateProject({ logistics: { ...l, parking } })} /><EditableArea label="Accessibilité" value={l.accessibility} onChange={accessibility => updateProject({ logistics: { ...l, accessibility } })} /><EditableArea label="Plan météo de repli" value={l.weatherFallback} onChange={weatherFallback => updateProject({ logistics: { ...l, weatherFallback } })} /><div className="rounded-2xl border border-white/10 p-4"><div className="flex justify-between items-center mb-3"><p className="text-[10px] uppercase tracking-widest text-white/40">À emporter</p><AddBar label="Ajouter" onAdd={() => updateProject({ logistics: { ...l, packing: [...l.packing, { id: newId(), label: "Nouvel élément", done: false }] } })} /></div>{l.packing.length === 0 ? <Empty>La liste est vide.</Empty> : l.packing.map(item => <div key={item.id} className="flex items-center gap-3 py-2 border-b border-white/5 last:border-0"><button onClick={() => updateProject({ logistics: { ...l, packing: l.packing.map(x => x.id === item.id ? { ...x, done: !x.done } : x) } })} className={cn("w-5 h-5 rounded border flex items-center justify-center", item.done ? "bg-white text-black" : "border-white/25")}>{item.done && <Check className="w-3 h-3" />}</button><input value={item.label} onChange={e => updateProject({ logistics: { ...l, packing: l.packing.map(x => x.id === item.id ? { ...x, label: e.target.value } : x) } })} className={cn("text-sm flex-1 bg-transparent outline-none", item.done && "line-through text-white/40")} /><button onClick={() => updateProject({ logistics: { ...l, packing: l.packing.filter(x => x.id !== item.id) } })} className="text-white/30 hover:text-rose-300"><Trash2 className="w-3.5 h-3.5" /></button></div>)}</div><div className="rounded-2xl border border-white/10 p-4"><div className="flex items-center justify-between mb-3"><p className="text-[10px] uppercase tracking-widest text-white/40">Contacts d'urgence</p><AddBar label="Ajouter" onAdd={() => updateProject({ logistics: { ...l, emergencyContacts: [...l.emergencyContacts, { id: newId(), name: "Nouveau contact", phone: "", role: "À préciser" }] } })} /></div>{l.emergencyContacts.map(contact => <div key={contact.id} className="grid grid-cols-3 gap-2 border-b border-white/5 py-2 last:border-0"><input value={contact.name} onChange={e => updateProject({ logistics: { ...l, emergencyContacts: l.emergencyContacts.map(x => x.id === contact.id ? { ...x, name: e.target.value } : x) } })} className="bg-transparent text-sm outline-none" /><input value={contact.phone} onChange={e => updateProject({ logistics: { ...l, emergencyContacts: l.emergencyContacts.map(x => x.id === contact.id ? { ...x, phone: e.target.value } : x) } })} placeholder="Téléphone" className="bg-transparent text-xs outline-none" /><input value={contact.role} onChange={e => updateProject({ logistics: { ...l, emergencyContacts: l.emergencyContacts.map(x => x.id === contact.id ? { ...x, role: e.target.value } : x) } })} className="bg-transparent text-xs text-white/50 outline-none" /></div>)}</div><div className="grid gap-3 md:grid-cols-2">{l.accommodations.map(a => <div key={a.id} className="rounded-2xl border border-white/10 p-4"><p className="text-sm">{a.name}</p><p className="text-xs text-white/45 mt-1">{a.booked} réservées · {a.address}</p></div>)}{l.shuttles.map(s => <div key={s.id} className="rounded-2xl border border-white/10 p-4"><p className="text-sm">{s.route}</p><p className="text-xs text-white/45 mt-1">Départ {s.departure} · {s.capacity} places</p></div>)}</div></div>;
  }
  ...
  return <CollectionPanel title="Souvenirs et après" addLabel="Ajouter un élément" onAdd={() => addEntity("memories", { kind: "shot", title: "Nouvelle idée", status: "a_faire" })}>{project.memories.length === 0 ? <Empty>Les souvenirs à préparer apparaîtront ici.</Empty> : project.memories.map(item => <div key={item.id} className="rounded-2xl border border-white/10 bg-white/[.035] p-4 flex items-center gap-3"><button onClick={() => updateEntity("memories", item.id, { status: item.status === "termine" ? "a_faire" : "termine" })} className={cn("w-6 h-6 rounded-full border flex items-center justify-center", item.status === "termine" ? "bg-white text-black" : "border-white/20")}>{item.status === "termine" && <Check className="w-3 h-3" />}</button><div className="grid flex-1 gap-2 sm:grid-cols-2"><input value={item.title} onChange={e => updateEntity("memories", item.id, { title: e.target.value })} className="bg-transparent text-sm outline-none" /><input value={item.owner || ""} onChange={e => updateEntity("memories", item.id, { owner: e.target.value })} placeholder="Responsable" className="bg-transparent text-xs text-white/55 outline-none" /><select value={item.kind} onChange={e => updateEntity("memories", item.id, { kind: e.target.value as MemoryItem["kind"] })} className="rounded-lg bg-white/10 px-2 py-1 text-xs outline-none"><option value="shot">Shot list</option><option value="media">Média</option><option value="message">Message</option><option value="album">Album</option><option value="rappel">Rappel</option></select></div><button onClick={() => removeEntity("memories", item.id)} className="text-white/30 hover:text-rose-300"><Trash2 className="w-4 h-4" /></button></div>)}</CollectionPanel>;
}
...
function GuestSeat({ guest, tables, onChange }: { guest: { name: string; tableId?: string; dietary?: string }; tables: { id: string; name: string }[]; onChange: (value: string) => void }) {
  return <div className="flex items-center gap-2 rounded-xl bg-black/20 px-3 py-2"><span className="text-sm flex-1 truncate">{guest.name}{guest.dietary && <span className="text-[10px] text-amber-300 ml-2">{guest.dietary}</span>}</span><select value={guest.tableId || ""} onChange={e => onChange(e.target.value)} className="max-w-[130px] rounded-lg bg-white/10 px-2 py-1.5 text-xs outline-none"><option value="">Sans table</option>{tables.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</select></div>;
}
...
function PaymentRow({ payment, onToggle, onDelete, onEdit }: { payment: Payment; onToggle: () => void; onDelete: () => void; onEdit: (u: Partial<Payment>) => void }) {
  return <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[.035] p-4"><button onClick={onToggle} className={cn("w-6 h-6 rounded-full border flex items-center justify-center", payment.state === "paye" ? "bg-emerald-300 text-black border-emerald-300" : "border-white/20")}>{payment.state === "paye" && <Check className="w-3.5 h-3.5" />}</button><div className="flex-1"><input value={payment.label} onChange={e => onEdit({ label: e.target.value })} className="bg-transparent text-sm outline-none w-full" /><p className="text-xs text-white/40 mt-1">{new Date(payment.at).toLocaleDateString("fr-FR")} · {payment.state === "paye" ? "réglé" : "à régler"}</p></div><input type="number" value={payment.amountCents / 100} onChange={e => onEdit({ amountCents: Number(e.target.value) * 100 })} className="w-24 rounded-lg bg-white/5 px-2 py-1.5 text-right font-mono text-sm outline-none" /><button onClick={onDelete} className="text-white/30 hover:text-rose-300"><Trash2 className="w-4 h-4" /></button></div>;
}

function EditableArea({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  return <label className="block rounded-2xl border border-white/10 bg-white/[.035] p-4"><span className="text-[10px] uppercase tracking-widest text-white/40">{label}</span><textarea value={value} onChange={e => onChange(e.target.value)} rows={3} className="mt-2 w-full resize-none bg-transparent text-sm leading-relaxed outline-none placeholder:text-white/20" placeholder="À compléter…" /></label>;
}
