...
export function PortalControls() {
  ...
  if (!project) return <button onClick={() => void signOut({ redirectUrl: basePath() })} className="fixed right-4 top-4 z-[60] rounded-full border border-foreground/20 bg-background/60 px-4 py-2 text-xs text-foreground backdrop-blur">Se déconnecter</button>;
  ...
  return <>
     <div data-testid="portal-controls" className="fixed right-3 top-[4.4rem] z-[60] flex items-center gap-2 sm:right-4 sm:top-4">
          <button data-testid="sync-status" title={syncError || `${reviewCount} élément${reviewCount === 1 ? '' : 's'} à vérifier`} onClick={() => setPanel('sync')} className="group flex h-10 sm:h-11 items-center gap-2 sm:gap-2.5 rounded-full bg-foreground px-3 sm:px-4 text-background shadow-lg transition hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label={`${reviewCount} élément${reviewCount === 1 ? '' : 's'} à vérifier`}>
            <span className="relative grid h-4 w-4 sm:h-5 sm:w-5 place-items-center">
              {reviewCount > 0 && <span className="absolute inset-0 animate-ping rounded-full bg-background/20" />}
              <SyncIcon className={`relative h-3 w-3 sm:h-3.5 sm:w-3.5 ${syncStatus === 'loading' || syncStatus === 'saving' ? 'animate-spin' : ''}`} />
            </span>
            <span className="hidden sm:inline text-[9px] font-semibold uppercase tracking-[.18em]">À vérifier</span>
            <span className="grid h-4 min-w-4 sm:h-5 sm:min-w-5 place-items-center rounded-full bg-background px-1 sm:px-1.5 text-[8px] sm:text-[9px] font-semibold text-foreground">{reviewCount}</span>
          </button>
         {canEdit && <button data-testid="settings-open" onClick={() => isProfileRoute ? window.dispatchEvent(new Event('aime:toggle-profile-editor')) : setPanel('editor')} className="flex h-10 sm:h-11 items-center gap-2 rounded-full border border-foreground/[.12] bg-background/90 px-3 sm:px-4 text-foreground/85 shadow-sm backdrop-blur-xl transition hover:border-foreground/30 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label={isProfileRoute ? 'Éditer le Profil' : 'Éditer le Monde'} title={isProfileRoute ? 'Éditer le Profil' : 'Éditer le Monde'}><PenLine className="h-3 w-3 sm:h-3.5 sm:w-3.5" /><span className="hidden sm:inline text-[9px] uppercase tracking-[.18em]">Éditer</span></button>}
    </div>
      {panel === 'sync' && <CenteredBlock eyebrow="Contrôle universel" title="Des changements sont à vérifier" description="AIME réunit ici la sauvegarde, les liens incomplets et les éléments qui demandent une décision humaine, quel que soit l’écran courant." onClose={() => setPanel(null)} size="lg">
        ...
        {syncError && <p className="mt-5 border-l border-destructive/50 py-1 pl-4 text-sm font-light leading-relaxed text-destructive/80">{syncError}</p>}
        {(audit.isolated.length > 0 || audit.dangling.length > 0 || audit.manualMusic.length > 0) && <div className="mt-6 space-y-1 border-t border-border pt-5">
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          <button onClick={() => setPanel('settings')} className="rounded-full bg-foreground px-4 py-2 text-xs font-medium text-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Ouvrir ME</button>
          <button onClick={() => setPanel(null)} className="rounded-full border border-foreground/15 px-4 py-2 text-xs text-foreground/70 hover:bg-foreground/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Fermer</button>
        </div>
      </CenteredBlock>}
      {panel === 'editor' && <CenteredBlock eyebrow="Éditeur du Monde" title="Modifier l’ouverture" description="Ces informations composent le hero et sa projection visible selon les droits de chacun." onClose={() => setPanel(null)} size="lg">
        <form className="space-y-5" onSubmit={event => {
          ...
          <button className="w-full rounded-full bg-foreground px-5 py-3 text-sm font-medium text-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Enregistrer l’ouverture</button>
        </form>
      </CenteredBlock>}
      {panel === 'settings' && <CenteredBlock eyebrow="ME" title="Votre espace" description={user?.primaryEmailAddress?.emailAddress} onClose={() => setPanel(null)} size="lg" testId="settings-panel">
        <label className="block text-xs uppercase tracking-widest text-foreground/40 mb-2">Projet actif</label>
        <select value={project.id} onChange={e => void selectProject(e.target.value)} className="w-full rounded-xl border border-border bg-card p-3 mb-6 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
          {projects.map(item => <option className="bg-background text-foreground" key={item.id} value={item.id}>{item.title} · {item.role}</option>)}
        </select>
         {notice && <p className="mb-5 border-l border-foreground/30 py-1 pl-3 text-sm text-foreground/60">{notice}</p>}
        <div className="grid grid-cols-2 gap-2">
          ...
          <button onClick={exportCsv} className="action focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Invités CSV</button>
          <button onClick={() => window.print()} className="action focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Imprimer Jour J / tables</button>
          <button onClick={() => document.getElementById('backup-input')?.click()} className="action focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Importer JSON</button>
        ...
        {files.length > 0 && <div className="mt-8 border-t border-border pt-6"><h3 className="font-medium mb-3">Documents & médias privés</h3><div className="space-y-2">{files.map(file =>
            <div key={file.id} className="flex items-center gap-2 rounded-xl bg-card p-3 text-xs border border-border"><span className="min-w-0 flex-1 truncate">{file.name}</span><a target="_blank" rel="noreferrer" href={`/api/storage/files/${file.id}`} className="text-foreground/70 hover:text-foreground">Aperçu</a><a href={`/api/storage/files/${file.id}?download=1`} className="text-foreground/70 hover:text-foreground">Télécharger</a>{canManage && <button className="text-foreground/40 transition hover:text-destructive" onClick={() => { setSelectedFile({ id: file.id, name: file.name }); setPanel('delete-file'); }}>Supprimer</button>}</div>
        )}</div></div>}
        <div className="mt-8 border-t border-border pt-6">
           <h3 className="font-medium mb-2">Confidentialité & conservation</h3>
           <p className="text-xs text-foreground/50 mb-3">Les documents restent privés. Choisissez la durée souhaitée. Pendant le pilote, aucune suppression automatique n’a lieu sans avertissement.</p>
           {currentRole === 'owner' && <select defaultValue="365" onChange={e => void api(`/projects/${project.id}/privacy`, { method: 'PATCH', body: JSON.stringify({ retentionDays: Number(e.target.value) }) }).then(() => setNotice('Préférence enregistrée')).catch(err => setNotice(err.message))} className="w-full rounded-xl border border-border bg-card p-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
            <option className="bg-background text-foreground" value="180">6 mois</option><option className="bg-background text-foreground" value="365">1 an</option><option className="bg-background text-foreground" value="1095">3 ans</option>
           </select>}
          {currentRole === 'owner' && <div className="mt-6 border-t border-border pt-6">
            ...
            {project.publicProfile?.published && <div className="mt-4 flex gap-2">
              <Link data-testid="link-public-profile" href={`/profil/${project.id}`} className="flex-1 rounded-full border border-foreground/15 px-4 py-2.5 text-center text-xs transition hover:bg-foreground/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Voir le profil</Link>
              ...
              }} className="flex-1 rounded-full bg-foreground px-4 py-2.5 text-xs font-medium text-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Copier le lien</button>
            </div>}
          </div>}
        </div>
        <div className="mt-8 space-y-2">
           <div className="flex justify-center gap-4 py-2 text-xs text-foreground/40"><Link href="/confidentialite">Confidentialité</Link><Link href="/conditions">Conditions</Link></div>
           <button data-testid="sign-out" className="w-full rounded-xl border border-border p-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" onClick={() => void signOut({ redirectUrl: basePath() })}>Se déconnecter</button>
           {currentRole === 'owner' && <button className="w-full p-3 text-sm text-foreground/40 transition hover:text-foreground/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" onClick={() => setPanel('delete-project')}>Supprimer définitivement le projet</button>}
           <button className="w-full p-3 text-sm text-destructive/60 transition hover:text-destructive focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" onClick={() => setPanel('delete-account')}>Supprimer mon compte et mes accès</button>
        </div>
       </CenteredBlock>}
      ...
      {panel === 'message' && <CenteredBlock eyebrow="ME · Messages" title="Préparer un message" description="AIME ne l’enverra qu’après votre confirmation." onClose={() => setPanel('settings')} size="lg">
        <form className="space-y-7" onSubmit={event => { event.preventDefault(); void sendMessage().catch(error => { setNotice(`E-mail non envoyé : ${error.message}`); setPanel('settings'); }); }}>
          ...
          <div className="border-l border-foreground/50 py-1 pl-4 text-xs font-light leading-relaxed text-foreground/50">Le message sera envoyé à {recipients.split(',').filter(value => value.trim()).length || 0} destinataire(s).</div>
      ...
      {panel === 'delete-file' && selectedFile && <CenteredBlock eyebrow="ME · Documents" title="Supprimer ce document ?" description={selectedFile.name} onClose={() => setPanel('settings')}>
        <p className="text-sm font-light leading-relaxed text-foreground/60">Le document ne sera plus disponible dans cet espace privé. Cette action ne peut pas être annulée.</p>
        <div className="mt-8 flex gap-3">
          <button onClick={() => setPanel('settings')} className="flex-1 rounded-full border border-foreground/50 px-5 py-3 text-sm text-foreground/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Conserver</button>
          <button onClick={() => void api(`/storage/files/${selectedFile.id}`, { method: 'DELETE' }).then(() => { setFiles(value => value.filter(item => item.id !== selectedFile.id)); setSelectedFile(null); setNotice('Document supprimé'); setPanel('settings'); }).catch(error => { setNotice(error.message); setPanel('settings'); })} className="flex-1 rounded-full bg-destructive px-5 py-3 text-sm font-medium text-destructive-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Supprimer</button>
        </div>
      </CenteredBlock>}
      {panel === 'delete-project' && <CenteredBlock eyebrow="ME · Monde" title="Supprimer définitivement ce Monde ?" description="Les informations, documents et accès associés seront supprimés." onClose={() => setPanel('settings')}>
        ...
        <div className="mt-8 flex gap-3">
          <button onClick={() => setPanel('settings')} className="flex-1 rounded-full border border-foreground/50 px-5 py-3 text-sm text-foreground/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Conserver</button>
          <button disabled={deleteConfirmation !== 'SUPPRIMER' || submitting} onClick={() => { setSubmitting(true); void api(`/projects/${project.id}`, { method: 'DELETE', body: JSON.stringify({ confirmation: 'SUPPRIMER' }) }).then(() => { clearProject(); location.reload(); }).catch(error => { setSubmitting(false); setNotice(error.message); setPanel('settings'); }); }} className="flex-1 rounded-full bg-destructive px-5 py-3 text-sm font-medium text-destructive-foreground disabled:cursor-not-allowed disabled:opacity-25 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Supprimer</button>
        </div>
      </CenteredBlock>}
      {panel === 'delete-account' && <CenteredBlock eyebrow="ME · Compte" title="Supprimer définitivement votre compte ?" description="Vos Mondes, leurs documents et tous vos accès seront supprimés. Cette action ne peut pas être annulée." onClose={() => setPanel('settings')}>
        ...
        <div className="mt-8 flex gap-3">
          <button onClick={() => setPanel('settings')} className="flex-1 rounded-full border border-foreground/50 px-5 py-3 text-sm text-foreground/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Conserver mon compte</button>
          ...
          }} className="flex-1 rounded-full bg-destructive px-5 py-3 text-sm font-medium text-destructive-foreground disabled:cursor-not-allowed disabled:opacity-25 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{submitting ? 'Suppression…' : 'Supprimer mon compte'}</button>
        </div>
      </CenteredBlock>}
  </>;
}
...
function ReviewCard({ label, value, detail, alert = false }: { label: string; value: number; detail: string; alert?: boolean }) {
  return <div className={`rounded-2xl border p-5 ${alert ? 'border-destructive/30 bg-destructive/10' : 'border-border bg-card'}`}>
    <div className="flex items-start justify-between gap-4">
      <p className="text-[10px] uppercase tracking-[.16em] text-foreground/50">{label}</p>
...
function ReviewLine({ label, meta }: { label: string; meta: string }) {
  return <div className="flex items-center justify-between gap-5 rounded-xl px-3 py-3 transition hover:bg-foreground/5">
    <span className="min-w-0 truncate text-sm text-foreground/90">{label}</span>
    <span className="shrink-0 text-[9px] uppercase tracking-[.14em] text-foreground/40">{meta}</span>
  </div>;
}

function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <label className="block"><span className="mb-2 block text-[10px] uppercase tracking-[.2em] text-foreground/40">{label}</span>{children}</label>;
}

function Actions({ onBack, submitLabel, disabled }: { onBack: () => void; submitLabel: string; disabled?: boolean }) {
  return <div className="flex gap-3 pt-2"><button type="button" onClick={onBack} className="flex-1 rounded-full border border-foreground/50 px-5 py-3 text-sm text-foreground/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Retour</button><button type="submit" disabled={disabled} className="flex-1 rounded-full bg-foreground px-5 py-3 text-sm font-medium text-background disabled:cursor-not-allowed disabled:opacity-25 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{submitLabel}</button></div>;
}
