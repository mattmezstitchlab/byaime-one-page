...
export type AgentCommand =
  ...
  | { kind: "remaining-tasks" }
  | { kind: "provider-schedule"; query?: string }
  | { kind: "dietary-analysis" }
  | { kind: "timeline-conflicts" }
  | { kind: "add-guests"; count: number };
...
export function parseFrenchCommand(input: string): AgentCommand | undefined {
  ...
  if (match) return { kind: "shift-event", query: match[1].trim(), minutes: Number(match[2]) };
  if (/(?:liste|voir|montre).*(?:tâches|taches).*(?:restantes?|à faire|a faire)/.test(normalized)) return { kind: "remaining-tasks" };
  match = normalized.match(/(?:prépare|prepare|génère|genere).*(?:planning|horaire).*(?:prestataire)(?:\s+(.+))?/);
  if (match) return { kind: "provider-schedule", query: match[1]?.trim() };
  if (/(?:analyse|besoins?).*(?:alimentaire|diététique|dietetique|régime|regime|allerg)/.test(normalized)) return { kind: "dietary-analysis" };
  if (/(?:cherche|trouve|analyse).*(?:conflits?).*(?:timeline|planning|horaire)?/.test(normalized)) return { kind: "timeline-conflicts" };
  match = normalized.match(/(?:ajoute|ajouter)\s+(\d+)\s+(?:invités|invites|personnes)/);
  if (match) return { kind: "add-guests", count: Number(match[1]) };
  return undefined;
}

export function proposeCommand(project: WorldProject, command: AgentCommand): CommandProposal {
...
export function executeCommand(project: WorldProject, proposal: CommandProposal, confirmed = false): { project: WorldProject; message: string } {
  if (proposal.mutation && !confirmed) throw new Error("Confirmation explicite requise");
  ...
  if (proposal.command.kind === "add-guests") {
    const guests = Array.from({ length: proposal.command.count }, (_, index) => ({
      id: `guest-${Date.now()}-${index}`, name: `Nouvel invité ${index + 1}`, role: "invite" as const, rsvp: "en_attente" as const,
