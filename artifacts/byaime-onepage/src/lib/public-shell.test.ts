 ...
 * n'a besoin ni de session, ni de base, ni d'API. Cette page est l'accueil :
 * l'ancienne vitrine `/agence` et `/monde` (la Bande, retirée le 16/09/2026) le
 * servent aussi. Ce test verrouille la liste des pages qui restent servies, et
 * le fait que tout le reste le dit explicitement au lieu de demander une
 * donnée.
 */

describe("resolveDegradedView — pages servies sans authentification", () => {
  it("sert l'accueil sur l'URL de la Bande retirée, la page unique du site public", () => {
    expect(resolveDegradedView("/monde")).toEqual({ kind: "landing" });
  ...
  it("sert l'accueil à la racine et sur l'ancienne vitrine, sans redirection vide", () => {
    // Un `<Redirect>` ne rend rien côté serveur : la page la plus exposée du
    // site serait blanche. La racine et `/agence` servent donc l'accueil
    // lui-même.
    expect(resolveDegradedView("/")).toEqual({ kind: "landing" });
    expect(resolveDegradedView("/agence")).toEqual({ kind: "landing" });
  });

  it("ignore la query et l'ancre avant de décider", () => {
    expect(resolveDegradedView("/agence?utm_source=instagram")).toEqual({ kind: "landing" });
    expect(resolveDegradedView("/agence#methode")).toEqual({ kind: "landing" });
