import { WorldEvent, WorldDimension, DIMENSIONS, DataProvider, WorldEventFilter } from "./types";
...
export function generateDemoEvents(count: number, baseTime: number): WorldEvent[] {
...
export class DemoProvider implements DataProvider {
  ...
  private applyFilter(events: WorldEvent[], filter?: WorldEventFilter) {
    if (!filter) return events;
    return events.filter(e => {
      if (filter.dimension && filter.dimension.length > 0 && !filter.dimension.includes(e.dimension)) return false;
      if (filter.continent && e.continent !== filter.continent) return false;
      if (filter.country && e.country !== filter.country) return false;
