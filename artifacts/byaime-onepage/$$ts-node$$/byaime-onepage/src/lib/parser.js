...
export function parseIntention(text) {
    ...
    if (monthMatch) {
        ...
        if (mIdx >= 0)
            pivotDate.setMonth(mIdx);
        pivotConfidence = "confirme";
    }
    let guests = null;
    let guestsConf = "manquant";
    ...
    if (/(mariage|marier|épouser)/i.test(text))
        universe = "Mariage";
    else if (/(anniversaire)/i.test(text))
        universe = "Anniversaire";
    else if (/(séminaire|seminaire|entreprise|team building)/i.test(text))
        universe = "Entreprise";
    else if (/(voyage|vacances|retraite)/i.test(text))
        universe = "Voyage";
...
export function createInitialProject(draft, intentionText) {
