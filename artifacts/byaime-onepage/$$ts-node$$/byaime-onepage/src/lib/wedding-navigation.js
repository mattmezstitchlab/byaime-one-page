...
export function getWeddingCapabilities(role) {
    switch (role) {
        case "owner":
        case "planner":
...
export function getInitialWorldPhase(pivotTime, currentTime = Date.now()) {
    const pivot = new Date(pivotTime);
    const current = new Date(currentTime);
    if (pivot.getFullYear() === current.getFullYear() && pivot.getMonth() === current.getMonth() && pivot.getDate() === current.getDate())
        return "pendant";
...
const item = (id, label, description, destination) => ({ id, label, description, destination });
const timeline = (label, description) => item("timeline", label, description, { kind: "view", view: "chronological" });
...
export function getWeddingNavigation(phase, capabilities) {
    ...
    const allowed = (entry) => {
        if (entry.id === "finances")
            return capabilities.seeFinances;
        if (entry.id === "documents" || entry.id === "film")
            return capabilities.managePrivateDocuments;
        if (full || operational)
            return true;
...
export const WEDDING_PANEL_LABELS = {
...
export function isWeddingDestinationActive(destination, view, panel) {
    if (destination.kind === "view")
        return destination.view === "music" ? destination.view === view && (panel === null || panel === "music") : panel === null && destination.view === view;
    return destination.kind === "panel" ? destination.panel === panel : false;
}
export function isWeddingPanelAvailable(panel, navigation, view) {
    if (panel === "sections")
        return true;
    const available = [...navigation.primary, ...navigation.secondary].some(item => item.destination.kind === "panel" && item.destination.panel === panel);
    if (available)
        return true;
    return panel === "music" && view === "music";
}
export function getWeddingNavigationLabel(view, panel, navigation) {
    if (panel)
        return WEDDING_PANEL_LABELS[panel];
...
//# sourceMappingURL=wedding-navigation.js.map