...
import { Play, Pause, RotateCcw, ChevronLeft, ChevronRight, MousePointer2, User, Globe2, Plus, Shield, PenLine, Settings, CheckCircle2 } from "lucide-react";
import { cn } from "@/lib/utils";
const FakeCursor = ({ x, y, active, reducedMotion }) => {
    if (reducedMotion)
        return null;
    return (<motion.div initial={false} animate={{ left: `${x}%`, top: `${y}%`, scale: active ? 0.9 : 1 }} transition={{ type: "spring", stiffness: 100, damping: 20, mass: 15 }} className="absolute z-50 pointer-events-none drop-shadow-xl" style={{ marginLeft: -12, marginTop: -12 }}>
      <MousePointer2 className="w-8 h-8 text-foreground fill-background"/>
      {active && (<motion.div initial={{ scale: 0, opacity: 0.8 }} animate={{ scale: 2, opacity: 0 }} transition={{ duration: 0.5 }} className="absolute inset-0 rounded-full border-2 border-foreground"/>)}
    </motion.div>);
};
// Fake UIs for Demos
const ProfileFakeUI = () => (<div className="flex flex-col items-center justify-center h-full p-8 text-center bg-card/50">
...
const WorldFakeUI = () => (<div className="flex flex-col h-full bg-card/50 p-6">
    ...
    <div className="flex-1 rounded-2xl border border-border bg-background p-4 relative overflow-hidden">
       ...
       <div className="space-y-6 mt-4 ml-6">
         {[1, 2, 3].map(i => (<div key={i} className="relative flex items-center gap-4">
...
const NetworkFakeUI = () => (<div className="flex items-center justify-center h-full bg-card/50 p-6 relative overflow-hidden">
...
const ActionPillFakeUI = ({ focus }) => (<div className="flex flex-col items-center justify-center h-full bg-card/50 p-6 relative">
    <div className="mb-12 w-full max-w-md">
       <AnimatePresence mode="wait">
         {focus === 'ai' && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="p-6 rounded-2xl border border-border bg-background shadow-xl">
         ...
         {focus === 'plus' && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="p-6 rounded-2xl border border-border bg-background shadow-xl">
         ...
         {focus === 'me' && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="p-6 rounded-2xl border border-border bg-background shadow-xl">
...
const CreateMondeFakeUI = ({ step }) => (<div className="flex flex-col items-center justify-center h-full p-8 bg-card/50 relative">
    <AnimatePresence mode="wait">
       {step === 0 && (<motion.div key="btn" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.05 }} className="p-4 rounded-2xl border border-border bg-background shadow-xl flex items-center gap-4">
       ...
       {step === 1 && (<motion.div key="form" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="w-full max-w-sm p-6 rounded-2xl border border-border bg-background shadow-xl">
       ...
       {step === 2 && (<motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.05 }} className="w-full max-w-sm p-8 rounded-2xl border border-border bg-background shadow-xl flex flex-col items-center text-center">
...
const RolesFakeUI = ({ role }) => (<div className="flex flex-col h-full bg-card/50 p-6 relative">
...
function ScriptedDemoPlayer({ config }) {
    ...
    return (<div className="flex flex-col overflow-hidden rounded-[2rem] border border-border bg-card/30 shadow-2xl">
      <div className="relative h-[400px] w-full bg-background overflow-hidden border-b border-border">
         <AnimatePresence mode="wait">
            <motion.div key={currentStep} initial={{ opacity: 0, filter: "blur(10px)" }} animate={{ opacity: 1, filter: "blur(0px)" }} exit={{ opacity: 0, filter: "blur(10px)" }} transition={{ duration: 0.6 }} className="absolute inset-0">
              {step.ui}
            </motion.div>
         </AnimatePresence>
      ...
      <div className="flex gap-1 p-2 bg-background/50">
        {config.steps.map((_, idx) => (<div key={idx} className="h-1 flex-1 rounded-full overflow-hidden bg-foreground/10">
             {idx === currentStep && isPlaying && !reducedMotion && (<motion.div initial={{ width: "0%" }} animate={{ width: "100%" }} transition={{ duration: 4.5, ease: "linear" }} className="h-full bg-foreground" data-testid="demo-progress-active"/>)}
             {(idx < currentStep || (idx === currentStep && !isPlaying) || (idx === currentStep && reducedMotion)) && (<div className="h-full bg-foreground w-full" data-testid="demo-progress-static"/>)}
          </div>))}
      </div>
    </div>);
}
export function GuidesPage() {
    ...
    return (<main data-testid="guides-page" className="min-h-[100dvh] bg-background text-foreground selection:bg-foreground/20">
      ...
      <section className="px-6 pb-40 max-w-6xl mx-auto grid lg:grid-cols-[280px_1fr] gap-12 items-start">
         <div className="sticky top-24 space-y-2 hidden lg:block">
            {DEMOS.map(demo => (<button key={demo.id} data-testid={`demo-select-${demo.id}-desktop`} onClick={() => setActiveDemo(demo.id)} className={cn("block w-full text-left px-5 py-4 rounded-2xl transition-all duration-300 focus-visible:ring-2 focus-visible:ring-foreground focus-visible:outline-none", activeDemo === demo.id ? "bg-foreground text-background shadow-xl" : "bg-card/50 text-foreground/60 hover:bg-foreground/5 hover:text-foreground")}>
         ...
         <div className="lg:hidden flex overflow-x-auto gap-3 pb-4 hide-scrollbar snap-x">
            {DEMOS.map(demo => (<button key={demo.id} data-testid={`demo-select-${demo.id}-mobile`} onClick={() => setActiveDemo(demo.id)} className={cn("shrink-0 w-72 text-left px-5 py-4 rounded-2xl transition-all duration-300 snap-center focus-visible:ring-2 focus-visible:ring-foreground focus-visible:outline-none", activeDemo === demo.id ? "bg-foreground text-background" : "bg-card/50 text-foreground/60")}>
         ...
         <div className="min-w-0">
            <AnimatePresence mode="wait">
               {DEMOS.map(demo => demo.id === activeDemo && (<motion.div key={demo.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}>
