...
import { describeLaboratoryContext, describeLaboratoryJourney, laboratoryStatusDescriptions, focusWorld, laboratoryStatusLabels, laboratoryTypeLabels, openLaboratory, } from "@/lib/laboratory";
export function LaboratoryPage() {
    ...
    const visibleItems = useMemo(() => canManage ? items : items.filter(item => item.authoredByCurrentUser), [canManage, items]);
    const updateStatus = async (feedbackId, status) => {
        ...
        try {
            ...
            if (!response.ok)
                throw new Error(body?.error || `Erreur ${response.status}`);
            setItems((current) => current.map((item) => item.id === feedbackId ? body : item));
        }
        catch (reason) {
    ...
    if (!project) {
        return (<main className="grid min-h-full place-items-center bg-background px-6 py-20 text-center text-foreground">
    ...
    return (<main className="min-h-full bg-background px-6 py-12 text-foreground md:px-10">
      <div className="mx-auto max-w-5xl">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => openLaboratory({ context: { route: "laboratory", source: "laboratory-page" } })} className="rounded-full bg-foreground px-5 py-3 text-xs font-medium text-background">
              Nouveau retour
            </button>
            <button type="button" onClick={() => void load()} className="inline-flex items-center gap-2 rounded-full border border-foreground/15 px-4 py-3 text-xs text-foreground/70 hover:bg-foreground/5">
        ...
        <div className="mt-8 rounded-3xl border border-foreground/10 bg-card/70 p-5">
          <div className="flex flex-wrap gap-3">
            <label className="text-xs text-foreground/50">
              ...
              <select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)} className="mt-2 block rounded-xl border border-border bg-background px-3 py-2 text-sm">
                <option value="">Tous</option>
                {Object.entries(laboratoryTypeLabels).map(([value, label]) => (<option key={value} value={value}>{label}</option>))}
              </select>
            </label>
            <label className="text-xs text-foreground/50">
              Où en est l’amélioration
              <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="mt-2 block rounded-xl border border-border bg-background px-3 py-2 text-sm">
                <option value="">Tous</option>
                {Object.entries(laboratoryStatusLabels).map(([value, label]) => (<option key={value} value={value}>{label}</option>))}
              </select>
            </label>
          </div>
        ...
        {error && (<p className="mt-6 rounded-2xl border border-destructive/20 bg-destructive/5 px-4 py-3 text-sm text-destructive/85">
            {error}
          </p>)}

        {busy && !visibleItems.length ? (<p className="mt-10 text-sm text-foreground/45">Chargement du Laboratoire…</p>) : visibleItems.length === 0 ? (<div className="mt-10 rounded-3xl border border-dashed border-foreground/12 px-6 py-12 text-center">
            ...
            {visibleItems.map((item) => {
                ...
                return (<article key={item.id} className="rounded-3xl border border-foreground/10 bg-card/70 p-5">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {canManage && (<select value={item.status} onChange={(event) => void updateStatus(item.id, event.target.value)} className="rounded-full border border-foreground/15 bg-background px-3 py-2 text-xs text-foreground/75">
                          {Object.entries(laboratoryStatusLabels).map(([value, label]) => (<option key={value} value={value}>{label}</option>))}
                        </select>)}
                    </div>
                  </div>
                  <div className="mt-4 grid gap-4 md:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
                    ...
                    {journey && (<p className="mt-3 text-sm leading-6 text-foreground/72">{journey}</p>)}
                    {contextLines.length > 0 && (<div className="mt-3 flex flex-wrap gap-2 text-[11px] text-foreground/45">
                        {contextLines.map((line) => (<span key={line} className="rounded-full border border-foreground/10 bg-foreground/[.03] px-3 py-1">
                            {line}
                          </span>))}
                      </div>)}
                    {!journey && contextLines.length === 0 && (<p className="mt-3 text-sm text-foreground/45">Aucun contexte léger n’était disponible pour ce retour.</p>)}
                    </div>
                  </div>
...
//# sourceMappingURL=Laboratory.jsx.map