...
import { MapPin, CalendarDays, User, FileText, Folder, Wallet, Calendar, Users, Image as ImageIcon, Network, BookOpen, Fingerprint, Plus, Pencil, ZoomIn, ZoomOut } from "lucide-react";
import { useClerk, useUser } from "@clerk/react";
import { useLocation, useParams } from "wouter";
import { getGetPublicProfileQueryKey, useGetPublicProfile } from "@workspace/api-client-react";
...
export function PublicProfilePage({ privatePreview: forcePrivatePreview = false }) {
    const params = useParams();
    const [, navigate] = useLocation();
    const { user } = useUser();
    const { openUserProfile } = useClerk();
    const { project, isHydrated, currentRole, canEdit, participantLinks, refreshParticipantLinks, updateProject, updateEntity, } = useProject();
    ...
    const privatePreview = useMemo(() => {
        if (!project || (!forcePrivatePreview && project.id !== params.projectId))
            return undefined;
        return {
            id: project.id,
            title: project.title,
    ...
    useEffect(() => {
        if (!isPrivatePreview || (currentRole !== "owner" && currentRole !== "planner"))
            return;
    ...
    if (isLoading || (!isHydrated && !profile)) {
        return (<main data-testid="profile-loading" className="flex min-h-[100dvh] items-center justify-center bg-background px-6 text-foreground">
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: [0, 1, 0] }} transition={{ duration: 2.5, repeat: Infinity, ease: "easeInOut" }} className="text-[10px] uppercase tracking-[.35em] text-muted-foreground">
          AIME · Ouverture des archives
        </motion.p>
      </main>);
    }
    if (error || !profile) {
        return (<main data-testid="profile-error" className="flex min-h-[100dvh] flex-col items-center justify-center bg-background p-6 text-center text-foreground">
    ...
    return (<main data-testid="public-profile-page" className="relative h-[100dvh] w-full overflow-hidden bg-background text-foreground">
      {/* Background Depth */}
      ...
      {viewMode === "fil" ? (<div className="w-full h-full overflow-y-auto pt-32 pb-24 px-6 animate-in fade-in duration-500 relative z-10">
        ...
        </div>) : (<div ref={scrollRef} className="absolute inset-0 z-10 overflow-x-auto overflow-y-hidden hide-scrollbar animate-in fade-in duration-500">
        <div className="relative h-full" style={{ width: `${5600 * zoom}px` }}>
          <div className="absolute inset-y-0 left-0 w-[5600px] origin-left" style={{ transform: `scale(${zoom})`, transformOrigin: "left center" }}>
          ...
          {/* Section Markers */}
          {SECTIONS.map(section => (<div key={section.id} className="absolute top-1/2 -translate-y-1/2" style={{ left: `${section.x}px` }}>
          ...
          <div className="absolute top-1/2 -translate-y-1/2" style={{ left: '400px' }}>
             <div className="relative flex flex-col items-center -translate-x-1/2 w-[600px]">
                <button type="button" onClick={() => isPrivatePreview && setSelectedNode({ type: "identity", label: "Identité" })} disabled={!isPrivatePreview} aria-label={isPrivatePreview ? "Modifier l’identité du Profil" : displayName} className={cn("relative group w-32 h-32 rounded-full border-2 border-foreground/15 bg-background flex items-center justify-center overflow-visible mb-8 shadow-2xl", isPrivatePreview && "transition-transform hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent")}>
                ...
                : <User className="w-10 h-10 text-foreground/20"/>}
                    {isPrivatePreview && (<div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                        <Pencil className="w-6 h-6 text-white"/>
                        <span className="text-[9px] uppercase tracking-widest text-white mt-2">Modifier</span>
                      </div>)}
                  </div>
                  {isPrivatePreview && (<div className="absolute bottom-0 right-0 w-8 h-8 bg-brand-accent rounded-full border-2 border-background flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                ...
                {profile.subtitle && (<h2 className="mb-8 max-w-[min(28rem,calc(100vw-3rem))] text-balance text-center text-base font-light leading-7 text-foreground/40 md:text-lg">
                    {profile.subtitle}
                  </h2>)}

                <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-3">
                  {profile.city && (<span className="flex items-center gap-2 text-[10px] uppercase tracking-widest text-foreground/40 bg-foreground/5 px-4 py-2 rounded-full border border-foreground/5">
                      <MapPin className="w-3.5 h-3.5"/> {profile.city}
                    </span>)}
                  {profile.pivot && (<span className="flex items-center gap-2 text-[10px] uppercase tracking-widest text-foreground/40 bg-foreground/5 px-4 py-2 rounded-full border border-foreground/5">
                      <CalendarDays className="w-3.5 h-3.5"/> {format(profile.pivot, "d MMM yyyy", { locale: fr })}
                    </span>)}
                </div>

                {isPrivatePreview && contextItems.length > 0 && (<div className="mt-8 flex max-w-full items-center justify-center -space-x-2 overflow-x-auto px-4 py-4 hide-scrollbar" aria-label="Collections du Monde">
                    {contextItems.map((item, index) => (<button key={item.id + index} type="button" onClick={() => setSelectedNode({ type: "item", collection: item._type, sourceRef: item, label: item._title })} title={item._title} className="relative group grid h-12 w-12 shrink-0 place-items-center rounded-full border-2 border-background bg-card text-foreground shadow-xl transition-transform hover:z-20 hover:-translate-y-1 hover:scale-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent" style={{ zIndex: 10 - index }}>
                    ...
                    {currentRole !== 'viewer' && (<button type="button" onClick={() => window.dispatchEvent(new Event("aime:open-create"))} title="Ajouter au Monde" className="relative z-10 grid h-12 w-12 shrink-0 place-items-center rounded-full border-2 border-background bg-brand-accent text-brand-accent-foreground shadow-[0_0_15px_hsl(var(--brand-accent)/0.4)] transition-transform hover:z-20 hover:-translate-y-1 hover:scale-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent">
          ...
          {sortedEvents.map((event, i) => {
                ...
                return (<div key={event.id} className="absolute top-1/2" style={{ left: `${x}px` }}>
                 <div className="absolute left-0 w-[1px] bg-foreground/15" style={{
                 ...
                 <button type="button" onClick={() => {
                    ...
                    }} aria-label={`Ouvrir le Moment ${event.title}`} className="absolute group flex flex-col items-center justify-center w-14 h-14 -translate-x-1/2 -translate-y-1/2" style={{ top: `${yOffset}px`, left: '0px' }}>
                    ...
                    <div className={cn("absolute flex flex-col items-center w-48 transition-opacity pointer-events-none", isTop ? "bottom-full mb-3" : "top-full mt-3")}>
                       ...
                       {event.relations && event.relations.length > 0 && (<div className="flex gap-1 mt-2">
                            {event.relations.map((rel, idx) => (<div key={idx} className="flex h-3.5 w-3.5 items-center justify-center rounded-full bg-foreground/10" title={rel.kind}>
          ...
          {guestArrivals.map(arrival => {
                ...
                return (<div key={arrival.id} className="absolute top-1/2" style={{ left: `${x}px` }}>
                <div className="absolute left-0 top-0 h-44 w-px -translate-y-full bg-brand-accent/25"/>
                <button type="button" onClick={() => setSelectedNode({
                        type: "item",
                        collection: "guests",
                        sourceRef: arrival.guest,
                        label: arrival.guest.name,
                    })} aria-label={`Ouvrir l’arrivée de ${arrival.guest.name}`} className="group absolute left-0 top-[-176px] flex w-44 -translate-x-1/2 flex-col items-center rounded-2xl border border-brand-accent/25 bg-background/85 px-3 py-3 text-center shadow-xl backdrop-blur-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent">
          ...
          {/* 3. ARCHIVES */}
          {isPrivatePreview && canEdit && (currentRole === "owner" || currentRole === "planner") && (<button type="button" onClick={() => navigate("/user-portal?create=document-media")} className="absolute top-1/2 flex -translate-x-1/2 -translate-y-1/2 items-center gap-3 rounded-full border border-foreground/12 bg-background/85 px-5 py-3 text-left shadow-xl backdrop-blur-md transition hover:border-foreground/30 hover:bg-foreground/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent" style={{ left: "4000px" }} aria-label="Ajouter ou remplacer un visuel du Monde">
          ...
          {archives.map((item, i) => {
                ...
                return (<div key={item.id || i} className="absolute top-1/2" style={{ left: `${x}px` }}>
                  <button onClick={() => isPrivatePreview && setSelectedNode({ type: "item", collection: item.collection, sourceRef: item, label: item.title || item.label })} className="absolute group flex flex-col items-center justify-center -translate-x-1/2 -translate-y-1/2 transition-transform hover:scale-110" style={{ top: `${yOffset}px`, left: '0px' }}>
          ...
          {guests.map((guest, i) => {
                ...
                const yOffset = Math.sin(angle) * radius;
                return (<div key={guest.id || i} className="absolute top-1/2" style={{ left: `${x}px` }}>
                  <button onClick={() => isPrivatePreview && setSelectedNode({ type: "item", collection: "guests", sourceRef: guest, label: guest.name })} className="absolute group flex flex-col items-center justify-center -translate-x-1/2 -translate-y-1/2 transition-transform hover:scale-110" style={{ top: `${yOffset}px`, left: '0px' }}>
      ...
      <header data-testid="profile-context-header" className="fixed left-1/2 top-20 z-[75] flex -translate-x-1/2 flex-col items-center justify-center gap-2 sm:flex-row md:top-8">
        {isPrivatePreview && (<div className="flex gap-1 border border-border/40 shadow-lg bg-background/88 px-1 py-1 backdrop-blur-xl rounded-full" role="tablist">
            <button onClick={() => setViewMode("timeline")} className={cn("whitespace-nowrap rounded-full px-4 py-1.5 text-[10px] font-medium uppercase tracking-[.14em] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground", viewMode === "timeline" ? "bg-foreground text-background shadow-md" : "text-foreground/60 hover:bg-foreground/10 hover:text-foreground")}>
              Timeline
            </button>
            <button onClick={() => setViewMode("fil")} className={cn("whitespace-nowrap rounded-full px-4 py-1.5 text-[10px] font-medium uppercase tracking-[.14em] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground", viewMode === "fil" ? "bg-foreground text-background shadow-md" : "text-foreground/60 hover:bg-foreground/10 hover:text-foreground")}>
              Le Fil
            </button>
          </div>)}

        {viewMode === "timeline" && (<nav className="flex items-center gap-1 rounded-full border border-border/40 bg-background/88 p-1 shadow-lg backdrop-blur-xl">
             {SECTIONS.map((section) => (<button key={section.id} onClick={() => {
                    setActiveSection(section.id);
                    scrollToSection(section.x);
                }} aria-current={activeSection === section.id ? "location" : undefined} className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[9px] uppercase tracking-widest transition-colors hover:bg-foreground/10 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent", activeSection === section.id ? "text-foreground bg-foreground/10" : "text-foreground/60")}>
                   <section.icon className="w-3 h-3"/>
                   <span className="hidden sm:inline">{section.label}</span>
                </button>))}
             <span className="mx-1 h-4 w-px bg-border" aria-hidden="true"/>
             <button type="button" onClick={() => setZoom(value => Math.max(0.75, Number((value - 0.15).toFixed(2))))} disabled={zoom <= 0.75} aria-label="Réduire la Timeline" className="grid h-7 w-7 place-items-center rounded-full text-foreground/55 transition hover:bg-foreground/10 hover:text-foreground disabled:opacity-30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent">
               <ZoomOut className="h-3.5 w-3.5"/>
             </button>
             <button type="button" onClick={() => setZoom(value => Math.min(1.3, Number((value + 0.15).toFixed(2))))} disabled={zoom >= 1.3} aria-label="Agrandir la Timeline" className="grid h-7 w-7 place-items-center rounded-full text-foreground/55 transition hover:bg-foreground/10 hover:text-foreground disabled:opacity-30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent">
      ...
      <AnimatePresence>
        {selectedNode && (<EntityEditor node={selectedNode} onClose={() => setSelectedNode(null)} project={project} updateProject={updateProject} updateEntity={updateEntity} openUserProfile={openUserProfile} currentRole={currentRole} canEdit={canEdit}/>)}
        {selectedPublicEvent && (<CenteredBlock eyebrow={format(selectedPublicEvent.time, "d MMMM yyyy", { locale: fr })} title={selectedPublicEvent.title} description="Moment publié en lecture seule." onClose={() => setSelectedPublicEvent(null)} leading={<EventIcon kind={selectedPublicEvent.kind} className="mt-4 h-6 w-6 text-foreground/50"/>}>
            <div className="mt-5 space-y-4 text-sm font-light leading-7 text-foreground/60">
              ...
              {selectedPublicEvent.location && (<p className="flex items-center gap-2">
...
//# sourceMappingURL=PublicProfile.jsx.map