...
describe("GuidesPage", () => {
    const Wrapper = ({ children }) => (<Router hook={() => ["/guides", () => { }]}>{children}</Router>);
    ...
    it("describes only the role boundaries enforced by the server", () => {
        const markup = renderToStaticMarkup(<Wrapper><GuidesPage /></Wrapper>);
        expect(markup).toContain("Quatre rôles font varier les actions disponibles et masquent les informations d’organisation sensibles.");
        expect(markup).not.toContain("chiffré");
        expect(markup).not.toContain("les autres invités");
    });
});
//# sourceMappingURL=guides.test.jsx.map