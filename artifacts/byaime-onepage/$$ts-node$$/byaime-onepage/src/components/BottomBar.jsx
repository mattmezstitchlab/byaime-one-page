...
export function BottomBar() {
    ...
    return (<>
      {/* Sliding Panel */}
      <AnimatePresence>
        {activePanel && (<motion.div initial={{ y: "100%", opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: "100%", opacity: 0 }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed bottom-[calc(3.5rem+env(safe-area-inset-bottom))] left-0 right-0 z-40 bg-zinc-950 border-t border-foreground/10 shadow-2xl h-[60vh] max-h-[500px] rounded-t-3xl text-foreground">
            <div className="absolute top-6 right-6">
              <button onClick={() => setActivePanel(null)} className="w-8 h-8 flex items-center justify-center rounded-full bg-foreground/10 hover:bg-foreground/20 transition-colors">
                <X className="w-4 h-4"/>
              </button>
            </div>
            ...
            <div className="p-8 h-full overflow-y-auto pb-20">
              ...
              <div className="grid gap-3 max-w-2xl mx-auto">
                {[1, 2, 3].map((i) => (<div key={i} className="p-4 rounded-xl border border-foreground/5 bg-foreground/5 flex items-center gap-4 group hover:bg-foreground/10 transition-colors cursor-pointer">
              ...
              {activePanel === 'chat' && (<div className="mt-12 flex justify-center opacity-20 grayscale">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 flex justify-center border-t border-border bg-background/95 pb-[env(safe-area-inset-bottom)] text-foreground backdrop-blur-xl">
        <div className="flex items-center justify-center gap-10 sm:gap-16 w-full max-w-3xl px-6 h-14">
          {navItems.map((item) => (<button key={item.id} onClick={() => togglePanel(item.id)} className={cn("p-2 flex items-center justify-center transition-colors relative group", activePanel === item.id ? "text-foreground" : "text-foreground/45 hover:text-foreground/80")}>
              <item.icon className="w-[20px] h-[20px] stroke-[1.5]"/>
              {activePanel === item.id && (<motion.div layoutId="active-indicator" className="absolute -top-[15px] h-[2px] w-full bg-foreground"/>)}
            </button>))}
        </div>
      </div>
    </>);
