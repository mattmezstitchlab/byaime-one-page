...
export function HeroSection() {
    ...
    return (<section className="relative w-full h-[100dvh] flex items-center justify-center overflow-hidden">
      {/* Background Image */}
      <div className="absolute inset-0 z-0 bg-cover bg-center bg-no-repeat transition-transform duration-1000" style={{ backgroundImage: `url(${getAssetUrl('images/visual-event-D_L9Q-iW.jpg')})` }}/>
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto px-4 sm:px-6 flex flex-col items-center text-center mt-20">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }} className="space-y-8 md:space-y-10 w-full">
          ...
          <div className="w-full flex justify-center px-2">
            ...
            <div className="w-full max-w-3xl glass-panel-dark rounded-[2rem] sm:rounded-full flex flex-col sm:flex-row items-center gap-2 border border-white/20 bg-black/40 p-1.5 text-white transition-all duration-300 hover:bg-black/55 sm:gap-0">
              ...
              <button onClick={() => document.getElementById('universes')?.scrollIntoView({ behavior: 'smooth' })} className="w-full sm:w-auto flex justify-between sm:justify-start items-center gap-3 px-5 py-3 rounded-full hover:bg-white/10 transition-colors whitespace-nowrap text-sm font-medium text-white/90">
              ...
              <input type="text" placeholder="Choisissez un sujet, puis répondez à une question à la fois." className="bg-transparent border-none outline-none w-full flex-1 px-5 py-3 text-sm placeholder:text-white/50 text-center sm:text-left min-w-0 text-white font-medium focus:placeholder:opacity-0 transition-opacity" value={currentQuestion} onChange={(e) => setCurrentQuestion(e.target.value)}/>
      ...
      <div className="absolute bottom-24 sm:bottom-32 left-0 right-0 z-10 flex flex-col items-center">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5, duration: 1 }} className="flex flex-col items-center gap-3 cursor-pointer opacity-50 hover:opacity-100 transition-opacity w-full px-6" onClick={() => document.getElementById('timeline')?.scrollIntoView({ behavior: 'smooth' })}>
