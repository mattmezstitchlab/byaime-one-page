...
import { filterTimeline } from '@/lib/timeline-graph';
import { consumeWorldFocus } from '@/lib/laboratory';
import { TimelineAudit } from './TimelineAudit';
import { CenteredBlock } from './CenteredBlock';
import { isWeddingDestinationActive, getWeddingCapabilities, getWeddingNavigation, isWeddingPanelAvailable, getInitialWorldPhase, } from '@/lib/wedding-navigation';
const CREATE_PANEL_TARGETS = {
    person: "guests",
    moment: "dayof",
    task: "planning",
...
function ProviderPortrait({ provider, index = 0 }) {
    const image = providerImages[provider.category] || 'images/visual-service-DXmeWatY.jpg';
    return (<span className="relative block h-9 w-9 overflow-hidden rounded-full border-2 border-black bg-zinc-800" style={{ zIndex: 10 - index }} title={provider.name || provider.role}>
      <img src={getAssetUrl(image)} alt="" className="h-full w-full object-cover"/>
    </span>);
}
function GuestPortrait({ guest, index = 0, large = false }) {
    const image = guestPortraitImages[index % guestPortraitImages.length];
    return (<span className={cn("relative block shrink-0 overflow-hidden rounded-full border-[3px] border-black bg-zinc-900 shadow-xl", large ? "h-20 w-20 sm:h-24 sm:w-24" : "h-14 w-14")} style={{ zIndex: 20 - index }} title={guest.name}>
      <img src={getAssetUrl(image)} alt={`Portrait de ${guest.name}`} className="h-full w-full scale-125 object-cover transition duration-500 hover:scale-[1.35]" style={{ objectPosition: `${30 + (index % 3) * 20}% center` }}/>
    </span>);
}
export function ProjectStage() {
    ...
    const [phase, setPhase] = useState(() => getInitialWorldPhase(pivotDate));
    ...
    const navigation = useMemo(() => getWeddingNavigation(phase, getWeddingCapabilities(currentRole)), [phase, currentRole]);
    useEffect(() => {
        if (!activePanel)
            return;
        if (!isWeddingPanelAvailable(activePanel, navigation, view))
            setActivePanel(null);
    }, [activePanel, navigation, view]);
    useEffect(() => {
        const openCreateTarget = (action) => {
            if (!action)
                return;
        ...
        const listener = (event) => openCreateTarget(event.detail);
        ...
        const requestedAction = new URLSearchParams(window.location.search).get("create");
    ...
    useEffect(() => {
        const applyFocus = (request) => {
            if (!request)
                return;
            if (request.phase === "avant" || request.phase === "pendant" || request.phase === "apres")
    ...
    const visibleEvents = useMemo(() => {
        ...
        return filterTimeline(project, view).filter(e => {
            // Phase filtering
            if (phase === 'avant' && e.phase !== 'avant')
                return false;
            if (phase === 'pendant' && e.phase !== 'pendant')
                return false;
            if (phase === 'apres' && e.phase !== 'apres')
                return false;
            return true;
        });
    }, [project, phase, view]);
    const countdownTargets = useMemo(() => {
        ...
        const targets = [
            ...
            ...project.tasks
                ...
                .map(task => ({ id: `task-${task.id}`, time: task.dueDate, title: task.title, kind: 'Échéance' })),
            ...project.timeline
    ...
    const stats = useMemo(() => {
        if (!project)
            return { booked: 0, open: 0, engaged: 0 };
    ...
    const subtitleIsRedundant = useMemo(() => {
        if (!project?.subtitle)
            return false;
        const normalize = (value) => value.toLocaleLowerCase('fr').replace(/[^a-zà-ÿ0-9]/g, '');
    ...
    const openWeddingDestination = (destination) => {
    ...
    const sectionsAreActive = activePanel === "sections"
        ...
        || view === "public-info";
    return (<div className="aime-world-surface relative min-h-screen bg-background text-foreground selection:bg-foreground/20 pb-32">
      {/* Cinematic Header */}
      <header className="relative isolate flex min-h-[75vh] w-full flex-col justify-start overflow-hidden px-6 pb-24 pt-32 sm:pt-40 md:px-12">
        <div data-preserve-color className="absolute inset-0 z-0 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `url(${getAssetUrl('images/visual-hotel-C8zQiMK2.jpg')})` }}/>
        ...
        <div className="aime-visual-copy relative z-20 mx-auto w-full max-w-5xl space-y-6">
          <motion.button type="button" onClick={() => setWorldMenuOpen(true)} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex w-fit items-center gap-2 rounded-full border border-white/20 bg-black/40 px-4 py-1.5 text-[11px] uppercase tracking-[0.2em] backdrop-blur-md transition hover:bg-white hover:text-black" aria-label="Choisir un Monde">
            {heroCopy.eyebrow}
            <ChevronDown className="h-3 w-3"/>
          </motion.button>

          <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-4xl sm:text-6xl md:text-7xl font-display font-medium tracking-tight">
            {heroCopy.title}
          </motion.h1>

          <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className={cn("min-h-12 max-w-2xl text-[15px] font-light leading-relaxed text-white/70 md:text-base", !heroCopy.description && "invisible")}>
            {heroCopy.description || "Le Monde reste à la même place."}
          </motion.p>

          {(isPublicInfo || phase !== "apres") && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="flex min-h-12 flex-wrap content-start gap-2 text-[13px]">
            <button type="button" onClick={() => setCalendarOpen(true)} className="flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-1.5 backdrop-blur-sm transition hover:border-white/30 hover:bg-white/10" aria-label="Ouvrir le calendrier du Monde">
              <CalendarDays className="h-3.5 w-3.5 text-white/55"/>
              {format(pivotDate, 'd MMMM yyyy', { locale: fr })}
            </button>
            {project.city.value && (<span className="rounded-full border border-white/10 bg-white/5 px-4 py-1.5 backdrop-blur-sm">
                {project.city.value}
              </span>)}
            {project.guestsCount.value && (<span className="rounded-full border border-white/10 bg-white/5 px-4 py-1.5 backdrop-blur-sm">
                {project.guestsCount.value} invités
              </span>)}
          </motion.div>}

          {!isPublicInfo && phase !== "apres" && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="flex flex-wrap gap-2 pt-4">
            <Link href="/network" className="group flex items-center gap-2 py-1 pr-2 text-xs text-white/80 transition hover:text-white" aria-label={`Ouvrir le Registre, ${project.guests.length + project.providers.length} personnes et professionnels`}>
            ...
            {stats.open > 0 && (<span className="rounded-full border border-white/20 bg-black/60 px-3 py-1 text-xs text-white/80">
                {stats.open} professionnels à trouver
              </span>)}
            {stats.engaged > 0 && (<span className="rounded-full border border-white/20 bg-black/60 px-3 py-1 text-xs text-white/80">
                {stats.engaged.toLocaleString('fr-FR')} € déjà prévus
              </span>)}
            <button type="button" onClick={() => setTasksOpen(true)} className="ml-auto grid h-14 w-14 shrink-0 place-items-center rounded-full p-[3px] transition hover:scale-105 focus:outline-none focus-visible:ring-2 focus-visible:ring-white" style={{ background: `conic-gradient(from -90deg, #ff375f 0deg, #ff9f0a ${completion * 1.2}deg, #ffe620 ${completion * 2.1}deg, #30d158 ${completion * 2.8}deg, #64d2ff ${completion * 3.25}deg, #bf5af2 ${completion * 3.6}deg, rgba(255,255,255,.14) ${completion * 3.6}deg 360deg)` }} aria-label={`Ouvrir les étapes, ${completion}% complété`}>
          ...
          {(isPublicInfo || phase === "avant") && <motion.button type="button" onClick={() => setCountdownsOpen(true)} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="mt-6 block w-full border-t border-white/10 pt-7 text-left transition hover:border-white/25" aria-label="Voir tous les comptes à rebours">
            ...
            {distanceToNext > 0 ? (<div className="mt-4 flex flex-wrap items-end gap-x-5 gap-y-2 font-display font-light tabular-nums text-white">
                {[
                    [nextDays, "jours"],
                    [nextHours, "heures"],
                    [nextMinutes, "minutes"],
                    [nextSeconds, "secondes"],
                ].map(([value, label]) => (<span key={label} className="inline-flex items-baseline gap-1.5">
              ...
              </div>) : (<p className="mt-3 font-display text-4xl font-light">Le Moment est arrivé</p>)}
          </motion.button>}
          {!isPublicInfo && phase === "pendant" && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="mt-6 border-t border-white/10 pt-7">
              <p className="text-[10px] uppercase tracking-[.24em] text-white/42">{liveEvent ? "En ce moment" : "Prochain Moment"}</p>
              {featuredDayEvent ? (<div className="mt-4 flex flex-wrap items-center gap-5">
                ...
                </div>) : <p className="mt-4 text-sm text-white/45">Ajoutez les Moments du Jour J pour activer le direct.</p>}
            </motion.div>)}
          {!isPublicInfo && phase === "apres" && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mt-6 grid max-w-2xl grid-cols-3 gap-6 border-t border-white/10 pt-7">
          ...
          {project.missing.length > 0 && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }} className="mt-3 flex max-w-xl items-start gap-3 text-xs text-white/55">
      ...
      <nav aria-label="Navigation principale du Mariage" className="sticky top-[57px] z-40 border-y border-border bg-card/95 backdrop-blur-xl sm:top-[61px]">
        <div className="mx-auto flex max-w-5xl items-center gap-2 overflow-x-auto px-3 py-3 hide-scrollbar sm:px-6">
          {navigation.primary.map(item => item.destination.kind === "route" ? (<Link key={item.id} href={item.destination.href} title={item.description} className="shrink-0 whitespace-nowrap rounded-full border border-foreground/10 px-4 py-2 text-[9px] uppercase tracking-[.13em] text-foreground/65 transition-colors hover:border-foreground/30 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
              {item.label}
            </Link>) : (<button key={item.id} type="button" onClick={() => openWeddingDestination(item.destination)} aria-current={isWeddingDestinationActive(item.destination, view, activePanel) ? "page" : undefined} aria-label={`${item.label} — ${item.description}`} className={cn("shrink-0 whitespace-nowrap rounded-full border px-4 py-2 text-[9px] uppercase tracking-[.13em] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", isWeddingDestinationActive(item.destination, view, activePanel)
      ...
      <main className="w-full">
        {view === "music" && (<section className="border-b border-border bg-card px-6 py-16">
        ...
        {view === "person" && (<section className="overflow-hidden border-b border-border bg-card px-6 py-20">
            <div className="mx-auto max-w-5xl">
              ...
              {project.guests.length ? (<div className="mt-14 flex flex-wrap items-end gap-x-2 gap-y-8 sm:gap-x-4">
                  {project.guests.map((guest, index) => (<Link key={guest.id} href="/network" className={cn("group flex flex-col items-center", index % 3 === 1 && "sm:translate-y-8")} aria-label={`Ouvrir ${guest.name} dans le Registre`}>
                ...
                </div>) : (<Link href="/network" className="mt-12 block rounded-3xl border border-dashed border-foreground/20 px-8 py-12 text-center text-sm text-foreground/50 hover:border-foreground/40 transition-colors">Le Registre accueillera ici les personnes reliées à ce Monde.</Link>)}
            </div>
          </section>)}
      ...
      {tasksOpen && (<CenteredBlock eyebrow="Progression du Monde" title={`${completion}% accompli`} description={`${project.tasks.filter(task => task.status === "termine").length} étape${project.tasks.filter(task => task.status === "termine").length > 1 ? "s" : ""} terminée${project.tasks.filter(task => task.status === "termine").length > 1 ? "s" : ""} sur ${project.tasks.length}.`} onClose={() => setTasksOpen(false)} size="lg" leading={<span className="grid h-14 w-14 shrink-0 place-items-center rounded-full p-[3px]" style={{ background: `conic-gradient(from -90deg, #ff375f 0deg, #ff9f0a ${completion * 1.2}deg, #ffe620 ${completion * 2.1}deg, #30d158 ${completion * 2.8}deg, #64d2ff ${completion * 3.25}deg, #bf5af2 ${completion * 3.6}deg, rgba(255,255,255,.14) ${completion * 3.6}deg 360deg)` }}>
          ...
          {project.tasks.length ? (<div className="divide-y divide-border">
              {[...project.tasks]
                    ...
                    .map(task => (<div key={task.id} className="flex items-start gap-4 py-4">
            ...
            </div>) : (<p className="py-10 text-center text-sm text-foreground/40">Aucune étape n’a encore été créée.</p>)}
        </CenteredBlock>)}
      {worldMenuOpen && (<CenteredBlock eyebrow="Mariage" title="Choisir un mariage" description="Chaque mariage garde ses invités, ses Moments et son organisation dans un Monde dédié." onClose={() => setWorldMenuOpen(false)} size="lg">
          <div className="divide-y divide-border">
            {projects.map(item => (<button key={item.id} onClick={() => {
      ...
      {calendarOpen && (<CenteredBlock eyebrow="Calendrier du Monde" title={format(calendarMonth, "MMMM yyyy", { locale: fr })} description="Le temps du Monde, ses Moments et sa date pivot réunis dans une seule vue." onClose={() => setCalendarOpen(false)} size="lg" leading={<span className="mt-4 grid h-11 w-11 shrink-0 place-items-center rounded-full border border-foreground/[.12] bg-foreground/[.04]"><CalendarDays className="h-5 w-5 text-foreground/65"/></span>}>
          ...
          <div className="mt-5 grid grid-cols-7 gap-1">
            ...
            {calendarDays.map(day => {
                ...
                return (<button type="button" key={day.toISOString()} onClick={() => setSelectedDate(day)} className={cn("relative aspect-square rounded-2xl text-sm transition", !isSameMonth(day, calendarMonth) && "text-foreground/16", isSameMonth(day, calendarMonth) && "text-foreground/58 hover:bg-foreground/[.06] hover:text-foreground", isSelected && "bg-white text-black hover:bg-white hover:text-black", isPivot && !isSelected && "ring-1 ring-inset ring-foreground/45")}>
                  {format(day, "d")}
          ...
          <div className="mt-6 grid gap-4 border-t border-foreground/[.08] pt-6 sm:grid-cols-[1fr_auto] sm:items-end">
            ...
            <button type="button" disabled={!canEdit || isSameDay(selectedDate, project.pivot.value)} onClick={() => {
      ...
      {countdownsOpen && (<CenteredBlock eyebrow="Temps du Monde" title="Tous les comptes à rebours" description="Les prochains rendez-vous, échéances, Moments et la date pivot, réunis sans les confondre avec la progression du Monde." onClose={() => setCountdownsOpen(false)} size="lg">
          {countdownTargets.length ? (<div className="divide-y divide-border">
              {countdownTargets.map((target, index) => (<article key={target.id} className="grid gap-3 py-5 sm:grid-cols-[auto_1fr_auto] sm:items-center">
            ...
            </div>) : <p className="py-10 text-center text-sm text-foreground/40">Aucun rendez-vous, Moment ou délai à venir.</p>}
        </CenteredBlock>)}
    </div>);
}
//# sourceMappingURL=ProjectStage.jsx.map