...
import { getWeddingNavigationLabel, isWeddingDestinationActive, WEDDING_MODULE_IDS, WEDDING_PANEL_LABELS, } from "@/lib/wedding-navigation";
export function BottomDock({ phase, view, activePanel, navigation, onPhaseChange, onViewChange, onPanelChange, }) {
    ...
    const previousPhase = phase === "avant" ? null : phase === "pendant" ? "avant" : "pendant";
    const nextPhase = phase === "avant" ? "pendant" : phase === "pendant" ? "apres" : null;
    const phaseLabel = { avant: "Avant", pendant: "Le Jour J", apres: "Après" };
    ...
    <div className="mx-auto max-w-5xl">
      ...
      {activePanel === "sections" && <div className="mx-auto max-w-3xl">{navigation.secondary.map(item => item.destination.kind === "route" ? (<Link key={item.id} href={item.destination.href} onClick={() => onPanelChange(null)} className="group flex w-full items-center gap-4 border-b border-border py-5 text-left text-foreground/75 transition hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
        ...
        </Link>) : (<button key={item.id} onClick={() => openDestination(item.destination)} aria-current={isWeddingDestinationActive(item.destination, view, activePanel) ? "page" : undefined} className="group flex w-full items-center gap-4 border-b border-border py-5 text-left text-foreground/75 transition hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
          <Grid2X2 className="h-4 w-4 shrink-0 text-foreground/40"/><span className="min-w-0 flex-1"><span className="block text-xs uppercase tracking-[.18em]">{item.label}</span><span className="mt-1.5 block text-xs font-light text-foreground/45">{item.description}</span></span><ChevronRight className="h-4 w-4 shrink-0 text-foreground/25 transition group-hover:translate-x-1"/>
        </button>))}</div>}
      {isModule && <WeddingModulesPanel module={activePanel}/>}
    </div>
...
//# sourceMappingURL=BottomDock.jsx.map