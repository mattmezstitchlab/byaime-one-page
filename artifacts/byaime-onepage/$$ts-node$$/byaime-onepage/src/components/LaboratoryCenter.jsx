...
import { LABORATORY_FEEDBACK_TYPES, cleanLaboratoryContext, consumeLaboratoryDraft, describeLaboratoryContext, describeLaboratoryJourney, laboratoryTypeLabels, openLaboratory, } from "@/lib/laboratory";
...
export function LaboratoryCenter() {
    ...
    const baseContext = useMemo(() => buildBaseLaboratoryContext({
        pathname: location,
    ...
    useEffect(() => {
        ...
        if (pending)
            applyDraft(pending);
        const listener = (event) => applyDraft(event.detail);
    ...
    const submit = async () => {
        if (!project?.id || message.trim().length < 3)
            return;
        setBusy(true);
        setError("");
        try {
    ...
    return open ? (<CenteredBlock eyebrow="Laboratoire" title="Vous observez quelque chose dans AIME" description="Le Laboratoire recueille vos retours volontaires. « À vérifier » reste séparé et réservé aux anomalies ou décisions détectées par AIME." onClose={() => setOpen(false)} size="lg">
      <div className="space-y-5">
        {!project && (<div className="rounded-2xl border border-amber-300/20 bg-amber-300/5 p-4 text-sm text-foreground/70">
        ...
        <label className="block">
          ...
          <select value={type} onChange={(event) => setType(event.target.value)} className="w-full rounded-2xl border border-border bg-card px-4 py-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
            {LABORATORY_FEEDBACK_TYPES.map((value) => (<option key={value} value={value}>
        ...
        <label className="block">
          ...
          <textarea value={message} onChange={(event) => setMessage(event.target.value)} rows={6} placeholder="Décrivez simplement ce que vous venez de vivre, remarquer ou ne pas comprendre." className="w-full rounded-3xl border border-border bg-card px-4 py-4 text-sm outline-none focus:border-foreground/25 focus:ring-1 focus:ring-foreground/20"/>
        </label>
        <div className="rounded-2xl border border-foreground/10 bg-card p-4">
          ...
          {describeLaboratoryJourney(context) && (<p className="mt-3 text-sm text-foreground/70">
              {describeLaboratoryJourney(context)}
            </p>)}
          <div className="mt-3 flex flex-wrap gap-2 text-xs text-foreground/55">
            {describeLaboratoryContext(context).map((line) => (<span key={line} className="rounded-full border border-foreground/10 bg-foreground/[.03] px-3 py-1">
                {line}
              </span>))}
            {describeLaboratoryContext(context).length === 0 && (<span className="text-foreground/35">Aucun contexte léger disponible.</span>)}
          </div>
          <p className="mt-3 text-xs text-foreground/40">
            Le contexte affiché aide à situer votre retour, sans rejouer toute votre navigation.
          </p>
        </div>
        {error && (<p className="rounded-2xl border border-destructive/20 bg-destructive/5 px-4 py-3 text-sm text-destructive/85">
            {error}
          </p>)}
        {success && (<div className="rounded-2xl border border-emerald-300/20 bg-emerald-300/5 p-4 text-sm text-foreground/70">
            <div className="flex items-start gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" disabled={!project || busy || message.trim().length < 3} onClick={() => void submit()} className="inline-flex rounded-full bg-foreground px-4 py-2.5 text-xs font-medium text-background disabled:opacity-40">
            {busy ? "Enregistrement…" : "Partager au Laboratoire"}
          </button>
          <button type="button" onClick={() => navigate("/laboratoire")} className="inline-flex items-center gap-2 rounded-full border border-foreground/15 px-4 py-2.5 text-xs text-foreground/70 hover:bg-foreground/5">
            <Send className="h-3.5 w-3.5"/> Voir mon parcours
          </button>
          <button type="button" onClick={() => openLaboratory({ context: baseContext })} className="inline-flex rounded-full border border-foreground/15 px-4 py-2.5 text-xs text-foreground/55 hover:bg-foreground/5">
...
//# sourceMappingURL=LaboratoryCenter.jsx.map