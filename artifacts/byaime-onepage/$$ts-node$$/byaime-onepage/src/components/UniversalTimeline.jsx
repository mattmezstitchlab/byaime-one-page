...
import { analyzeEventImpact, applyPropagationPlan, planEventPropagation } from "@/lib/timeline-graph";
import { cn } from "@/lib/utils";
import { getAssetUrl } from "@/lib/assets";
import { ContextPanel } from "@/components/ContextPanel";
const kinds = ["guest", "table", "provider", "task", "payment", "document", "music", "team", "message", "logistics", "memory"];
const MONTH = 30 * 86400000;
const WEEK = 7 * 86400000;
const DAY = 86400000;
const HOUR = 3600000;
function getSubchapter(event, pivotTime) {
    const diff = event.time - pivotTime;
    if (diff < -24 * MONTH)
        return "L'Idée & La Vision";
    if (diff < -18 * MONTH)
        return "18 à 24 mois avant";
    if (diff < -12 * MONTH)
        return "12 à 18 mois avant";
    if (diff < -9 * MONTH)
        return "9 à 12 mois avant";
    if (diff < -6 * MONTH)
        return "6 à 9 mois avant";
    if (diff < -3 * MONTH)
        return "3 à 6 mois avant";
    if (diff < -1 * WEEK)
        return "1 à 3 mois avant";
    if (diff < -1 * DAY)
        return "La dernière ligne droite";
    if (diff < 0 && event.phase === "avant")
        return "La veille";
    if (event.phase === "pendant") {
        if (diff < 9 * HOUR)
            return "Le réveil";
        if (diff < 12 * HOUR)
            return "Les préparatifs";
        if (diff < 14 * HOUR)
            return "Mise en place";
        if (diff < 15.5 * HOUR)
            return "L'arrivée des invités";
        if (diff < 17 * HOUR)
            return "La cérémonie";
        if (diff < 17.5 * HOUR)
            return "Après la cérémonie";
        if (diff < 20 * HOUR)
            return "Le cocktail";
        if (diff < 22.5 * HOUR)
            return "Le repas";
        if (diff < 23.5 * HOUR)
            return "L'ouverture du bal";
        if (diff < 26 * HOUR)
            return "La soirée";
...
const AmbientBackground = ({ index }) => {
    const prefersReducedMotion = useReducedMotion();
    // Alternate every other scene with an image
    if (index % 2 === 0) {
        const imgIndex = (index / 2) % images.length;
        return (<div className="absolute inset-0 z-0 bg-cover bg-center bg-no-repeat overflow-hidden" style={{ backgroundImage: `url(${getAssetUrl(images[imgIndex])})` }}>
    ...
    return (<div className={cn("absolute inset-0 z-0 bg-gradient-to-b overflow-hidden", bg)}>
      ...
      <motion.div className="absolute inset-0 opacity-20" initial={{ opacity: 0.1 }} animate={prefersReducedMotion ? {} : {
            backgroundPosition: ['0% 0%', '100% 100%', '0% 0%'],
        }} transition={{ duration: 40, repeat: Infinity, ease: "linear" }} style={{
            backgroundImage: 'radial-gradient(circle at center, rgba(255,255,255,0.03) 0%, transparent 60%)',
            backgroundSize: '150% 150%'
        }}/>
    </div>);
};
function SubchapterTransition({ title }) {
    return (<div className="w-full py-24 flex items-center justify-center bg-background text-foreground relative z-10 border-t border-foreground/5">
       <h2 className="text-sm tracking-[0.4em] uppercase text-foreground/40">{title}</h2>
    </div>);
}
function EventScene({ event, index, onClick }) {
    return (<button onClick={onClick} className="relative w-full min-h-[60vh] flex items-center justify-center overflow-hidden border-t border-white/5 px-6 py-24 text-center text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-white/50 group">
      <AmbientBackground index={index}/>

      <div className="relative z-10 w-full max-w-4xl mx-auto flex flex-col items-center">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="space-y-6 flex flex-col items-center rounded-3xl p-8 md:p-12 bg-black/25 backdrop-blur-sm border border-white/10 hover:bg-black/40 transition-colors">
          ...
          {event.detail && (<p className="text-lg md:text-xl text-white/80 font-light max-w-2xl text-balance leading-relaxed">
              {event.detail}
            </p>)}

          <div className="flex flex-wrap justify-center gap-x-7 gap-y-3 pt-8">
            {event.location && (<span className="flex items-center gap-2 text-[10px] uppercase tracking-[.16em] text-white/60">
                <MapPin className="w-3 h-3"/>
                {event.location}
              </span>)}
            {(event.relations?.length || 0) > 0 && (<span className="flex items-center gap-2 text-[10px] uppercase tracking-[.16em] text-white/60">
...
export function UniversalTimeline({ events }) {
    ...
    return (<div className="w-full flex flex-col bg-background">
      {events.length === 0 && (<div className="py-32 text-center text-sm text-foreground/40">
      ...
      {canEdit ? (<div className="w-full py-32 flex justify-center bg-background border-t border-foreground/5">
          <button onClick={add} className="group relative flex h-16 w-16 items-center justify-center rounded-full border border-foreground/20 bg-card transition-colors hover:bg-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label="Ajouter un jalon">
            <Plus className="h-6 w-6 text-foreground transition-colors group-hover:text-background"/>
          </button>
        </div>) : (<div className="w-full py-20 flex justify-center bg-background border-t border-foreground/5">
          <p className="text-xs text-foreground/40 tracking-widest uppercase">Lecture seule</p>
        </div>)}

      {event && (<EventDrawer event={event} project={project} currentRole={currentRole} onClose={() => setSelected(undefined)} onEdit={updates => updateEntity("timeline", event.id, updates)} onApplyRipple={(plan, dependentIds) => {
                setUndoTimeline(project.timeline);
                updateProject({ timeline: applyPropagationPlan(project, plan, true, dependentIds).timeline });
            }} onDelete={() => { removeEntity("timeline", event.id); setSelected(undefined); }} canEdit={canEdit}/>)}
      {undoTimeline && (<div className="fixed bottom-24 left-4 z-[60] flex items-center gap-3 rounded-full border border-foreground/10 bg-background/90 py-2 pl-4 pr-2 text-xs text-foreground shadow-xl backdrop-blur sm:left-6">
...
function EventDrawer({ event, project, currentRole, onClose, onEdit, onApplyRipple, onDelete, canEdit }) {
    ...
    return (<ContextPanel eyebrow="Votre contexte" title="Moment du calendrier" onClose={onClose}>
        <div className="space-y-8">
          ...
          {ripplePlan && (<section className="overflow-hidden rounded-2xl border border-violet-300/20 bg-[linear-gradient(145deg,rgba(124,58,237,.14),rgba(14,165,233,.06))]">
              ...
              <div className="space-y-3 p-4">
                ...
                {ripplePlan.dependentChanges.length > 0 && (<div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              <select disabled={!canEdit} className={select} value={event.status} onChange={e => onEdit({ status: e.target.value })}>
            ...
            <div>
              ...
              <select disabled={!canEdit} className={select} value={event.provenance || "real"} onChange={e => onEdit({ provenance: e.target.value })}>
          ...
          <div>
            ...
            <select disabled={!canEdit} className={select} value={event.visibility || "equipe"} onChange={e => onEdit({ visibility: e.target.value })}>
          ...
          <div className="pt-4">
            ...
            <div className="space-y-3">
              {(event.relations || []).map((relation, index) => (<div key={`${relation.kind}-${relation.id}-${index}`} className="flex gap-3 items-end">
                  <select disabled={!canEdit} className={cn(select, "w-1/3")} value={relation.kind} onChange={e => onEdit({ relations: event.relations?.map((item, i) => i === index ? { ...item, kind: e.target.value } : item) })}>
            ...
            {canEdit && (<button onClick={() => onEdit({ relations: [...(event.relations || []), { kind: "guest", id: "" }] })} className="mt-4 text-[11px] uppercase tracking-widest text-foreground/50 hover:text-foreground transition-colors">
                + Ajouter un lien
              </button>)}
          </div>
          ...
          {canEdit && (<div className="pt-8 border-t border-foreground/10">
...
//# sourceMappingURL=UniversalTimeline.jsx.map