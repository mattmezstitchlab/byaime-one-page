...
const MAP_STYLE = {
    ...
    layers: [
        { id: 'carto-background', type: 'background', paint: { 'background-color': '#020202' } },
...
function toGeoJson(events, activeId) {
    return {
...
export function WorldMap({ events, activeId, onSelect }) {
        ...
        void (async () => {
            try {
                ...
                map.on('error', (event) => {
                    ...
                    if (/webgl|context lost|gpu/i.test(message))
                        activateFallback();
                });
                ...
                map.on('style.load', () => {
                    ...
                    if (!window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
                        let t = 0;
                        function animate() {
                            if (!mapRef.current || !mapRef.current.getLayer('events-pulse'))
                                return;
                            t += 0.05;
                    ...
                    map.on('click', 'events-core', (event) => {
    ...
    if (useFallback) {
        return (<div className="absolute inset-0 overflow-hidden bg-[#020202]" role="region" aria-label="Projection mondiale de secours des événements simulés">
        ...
        {events
                ...
                return (<button key={event.id} type="button" onClick={() => onSelect(event.id)} aria-label={`${event.title}, ${event.location ?? event.country ?? "Monde"}`} className="absolute -translate-x-1/2 -translate-y-1/2 rounded-full focus:outline-none focus-visible:ring-2 focus-visible:ring-white" style={{ left: `${left}%`, top: `${top}%` }}>
                <span className="absolute inset-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full opacity-20 animate-ping motion-reduce:animate-none" style={{
                        width: isActive ? 32 : 18,
                        height: isActive ? 32 : 18,
                        backgroundColor: DIMENSION_COLORS[event.dimension] || "#ffffff",
                    }}/>
                <span className="relative block rounded-full border border-black" style={{
                        width: isActive ? 9 : 5,
                        height: isActive ? 9 : 5,
                        backgroundColor: DIMENSION_COLORS[event.dimension] || "#ffffff",
                    }}/>
    ...
    return (<div ref={containerRef} className="absolute inset-0 bg-[#020202]" role="region" aria-label="Carte mondiale interactive des événements simulés"/>);
}
//# sourceMappingURL=WorldMap.jsx.map