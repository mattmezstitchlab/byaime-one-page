import { Activity, AlertCircle, ArrowRight, BookOpen, CheckCircle2, CheckSquare, Compass, RotateCcw, ShieldAlert, Sparkles, } from "lucide-react";
import { useGetProfileFil, } from "@workspace/api-client-react";
...
export function ProfileFilCardView({ card, onAction }) {
    const meta = typeMeta[card.type];
    const Icon = meta.icon;
    const actionable = card.action.kind !== "none";
    return (<article data-testid={`profile-fil-card-${card.id}`} className="group relative flex h-full flex-col gap-4 rounded-[1.75rem] border border-foreground/8 bg-card/70 p-5 shadow-sm transition duration-300 hover:-translate-y-0.5 hover:border-foreground/18 hover:bg-card md:p-6">
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1 pt-0.5">
          <div className="mb-2 flex flex-wrap items-center gap-2">
            ...
            {(card.priority === "urgent" || card.priority === "high") && (<span className="rounded-full border border-rose-400/20 bg-rose-400/8 px-2 py-0.5 text-[8px] uppercase tracking-[.16em] text-rose-300">
                Prioritaire
              </span>)}
          </div>
      ...
      <div className="ml-[3.75rem] border-l border-foreground/10 py-0.5 pl-4">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2 text-[9px] text-foreground/38">
          ...
          {card.evidenceStatus === "verified" && (<span className="flex items-center gap-1 uppercase tracking-[.14em] text-emerald-400/65">
      ...
      <div className="mt-auto pl-[3.75rem] pt-1">
        {actionable ? (<button type="button" onClick={() => onAction(card.action)} className="inline-flex items-center gap-2 rounded-full bg-foreground px-4 py-2.5 text-[9px] font-semibold uppercase tracking-[.15em] text-background transition hover:opacity-85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
            {card.action.label} <ArrowRight className="h-3.5 w-3.5"/>
          </button>) : (<span className="text-[9px] uppercase tracking-[.16em] text-foreground/30">{card.action.label}</span>)}
      </div>
    </article>);
}
export function ProfileFil({ projectId, onOpenMoment }) {
    const [, navigate] = useLocation();
    const { data, isLoading, error, refetch } = useGetProfileFil(projectId);
    if (isLoading) {
        return (<div data-testid="profile-fil-loading" className="flex flex-col items-center justify-center py-28 text-center">
    ...
    if (error || !data) {
        return (<div data-testid="profile-fil-error" className="flex flex-col items-center justify-center px-6 py-28 text-center">
    ...
    if (data.cards.length === 0) {
        return (<div data-testid="profile-fil-empty" className="flex flex-col items-center justify-center px-6 py-28 text-center">
    ...
    const order = ["now", "world", "learn", "inspiration"];
    const groups = order
    ...
    return (<div data-testid="profile-fil" className="mx-auto w-full max-w-5xl px-5 pb-28 md:px-8">
      ...
      <div className="space-y-14">
        {groups.map(group => (<section key={group.category} aria-labelledby={`profile-fil-${group.category}`}>
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              {group.cards.map(card => (<ProfileFilCardView key={card.id} card={card} onAction={action => {
                    if (action.kind === "open_timeline" && action.targetId)
                        onOpenMoment(action.targetId);
                    if (action.kind === "open_world")
      ...
      {!hasPublicInspiration && (<aside className="mt-14 rounded-2xl border border-dashed border-foreground/10 px-5 py-4 text-xs font-light leading-6 text-foreground/40">
...
//# sourceMappingURL=ProfileFil.jsx.map