...
import { UNIVERSAL_CREATE_ACTIONS } from "@/lib/universal/create-actions";
...
export function GlobalCreateCenter({ destination }) {
    ...
    if (!open)
        return null;
    const availableActions = UNIVERSAL_CREATE_ACTIONS.filter(action => action.availableInCurrentProject && action.id in actionIcons);
    const openAction = (id) => {
        if (destination === "world") {
            window.dispatchEvent(new CustomEvent("aime:open-create-target", { detail: id }));
        }
        else {
    ...
    return (<CenteredBlock eyebrow="+ · Créer ou relier" title={project ? `Ajouter dans ${project.title}` : "Créer votre premier Monde"} description={project ? contextCopy[destination] : "Un Monde réunit les Moments, les personnes et les informations qui donnent vie à votre projet."} onClose={() => setOpen(false)} size="lg" leading={<span className="mt-3 grid h-11 w-11 shrink-0 place-items-center rounded-full bg-foreground text-background"><Plus className="h-5 w-5"/></span>}>
      {!project ? (<button type="button" onClick={() => {
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {availableActions.map(action => {
                ...
                return (<button key={action.id} type="button" disabled={!canUse} onClick={() => openAction(action.id)} className="group flex min-h-32 items-start gap-4 rounded-2xl border border-foreground/10 bg-foreground/[.035] p-5 text-left transition hover:border-foreground/25 hover:bg-foreground/[.07] disabled:cursor-not-allowed disabled:opacity-35 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
...
//# sourceMappingURL=GlobalCreateCenter.jsx.map