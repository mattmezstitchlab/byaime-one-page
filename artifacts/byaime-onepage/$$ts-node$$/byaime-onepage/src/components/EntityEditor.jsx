...
export function EntityEditor({ node, onClose, project, updateProject, updateEntity, openUserProfile, currentRole, canEdit }) {
    ...
    if (node.type === "identity") {
        return (<CenteredBlock eyebrow="Édition" title="Identité du Profil" onClose={onClose} leading={<User className="mt-4 w-6 h-6 text-foreground/50"/>}>
         <div className="space-y-8 mt-4">
            ...
            {currentRole === "owner" ? <div className="pt-6 border-t border-foreground/10">
               ...
               <div className="flex items-center gap-3">
                  <button onClick={() => updateProject({ publicProfile: { published: false } })} className={cn("flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border transition-all", !project.publicProfile?.published ? "bg-foreground/10 border-foreground/30 text-foreground" : "bg-transparent border-foreground/10 text-foreground/40 hover:bg-foreground/5")}>
                     <Lock className="w-4 h-4"/> Privé
                  </button>
                  <button onClick={() => updateProject({ publicProfile: { published: true } })} className={cn("flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border transition-all", project.publicProfile?.published ? "bg-foreground/10 border-foreground/30 text-foreground" : "bg-transparent border-foreground/10 text-foreground/40 hover:bg-foreground/5")}>
    ...
    if (node.id === "worlds" || node.collection === "project") {
        return (<CenteredBlock eyebrow="Édition" title="Le Monde" onClose={onClose} leading={<Globe className="mt-4 w-6 h-6 text-foreground/50"/>}>
         <div className="space-y-6 mt-4">
            <div>
               <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Titre du Monde</label>
               <input type="text" className={inputClass} defaultValue={project.title} disabled={!canEdit} onBlur={(e) => updateProject({ title: e.target.value })}/>
            </div>
            <div>
                <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Sous-titre du Monde</label>
               <input type="text" className={inputClass} defaultValue={project.subtitle || ""} disabled={!canEdit} onBlur={(e) => updateProject({ subtitle: e.target.value })}/>
            </div>
            <div>
                <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Date pivot du Monde</label>
               <input type="date" className={inputClass} defaultValue={project.pivot?.value ? format(project.pivot.value, "yyyy-MM-dd") : ""} disabled={!canEdit} onChange={(e) => {
                if (e.target.value) {
    ...
    if (node.id === "map") {
        return (<CenteredBlock eyebrow="Édition" title="Carte & Lieux" onClose={onClose} leading={<MapPin className="mt-4 w-6 h-6 text-foreground/50"/>}>
         <div className="space-y-6 mt-4">
            <div>
                <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Ville du Monde</label>
               <input type="text" className={inputClass} defaultValue={project.city?.value || ""} disabled={!canEdit} onBlur={(e) => updateProject({ city: { ...project.city, value: e.target.value || null } })}/>
            </div>
            <div>
                <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Lieu principal</label>
               <input type="text" className={inputClass} defaultValue={project.venue?.value || ""} disabled={!canEdit} onBlur={(e) => updateProject({ venue: { ...project.venue, value: e.target.value || null } })}/>
            </div>
         </div>
      </CenteredBlock>);
    }
    if (node.type === "item" && node.collection === "timeline") {
        ...
        return (<CenteredBlock eyebrow="Édition du Moment" title={event.title} onClose={onClose} leading={<Calendar className="mt-4 w-6 h-6 text-foreground/50"/>}>
         <div className="space-y-6 mt-4">
            <div>
               <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Titre du moment</label>
               <input type="text" className={inputClass} defaultValue={event.title} disabled={!canEdit} onBlur={(e) => updateEntity("timeline", event.id, { title: e.target.value })}/>
            </div>
            <div>
               <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Détails et références</label>
               <textarea className={cn(inputClass, "min-h-[100px] resize-none")} defaultValue={event.detail || ""} disabled={!canEdit} onBlur={(e) => updateEntity("timeline", event.id, { detail: e.target.value })}/>
            </div>
            <div className="grid grid-cols-2 gap-4">
               <div>
                  <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Date</label>
                  <input type="date" className={inputClass} defaultValue={event.time ? format(event.time, "yyyy-MM-dd") : ""} disabled={!canEdit} onChange={(e) => {
                if (e.target.value) {
               ...
               <div>
                  ...
                  <input type="text" className={inputClass} defaultValue={event.location || ""} disabled={!canEdit} onBlur={(e) => updateEntity("timeline", event.id, { location: e.target.value })}/>
               </div>
            </div>

            {canEdit && (<div className="pt-6 border-t border-foreground/10">
                  ...
                  <div className="flex items-center gap-2 bg-foreground/5 p-1 rounded-xl border border-foreground/10">
                     {["prive", "equipe", "audience"].map(v => (<button key={v} onClick={() => updateEntity("timeline", event.id, { visibility: v })} className={cn("flex-1 py-2 rounded-lg text-xs font-medium transition-colors flex justify-center items-center gap-2", event.visibility === v ? "bg-foreground/15 text-foreground shadow-sm" : "text-foreground/40 hover:text-foreground/80")}>
    ...
    if (node.type === "item" && node.collection === "documents") {
        ...
        return (<CenteredBlock eyebrow="Édition Archive" title={doc.title} onClose={onClose} leading={<Folder className="mt-4 w-6 h-6 text-foreground/50"/>}>
           <div className="space-y-6 mt-4">
              <div>
                 <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Titre</label>
                 <input type="text" className={inputClass} defaultValue={doc.title} disabled={!canEdit} onBlur={(e) => updateEntity("documents", doc.id, { title: e.target.value })}/>
              </div>
              <div className="grid grid-cols-2 gap-4">
                 <div>
                    <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Type</label>
                    <select className={selectClass} defaultValue={doc.kind} disabled={!canEdit} onChange={(e) => updateEntity("documents", doc.id, { kind: e.target.value })}>
                 ...
                 <div>
                    <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Date</label>
                    <input type="date" className={inputClass} defaultValue={doc.at ? format(doc.at, "yyyy-MM-dd") : ""} disabled={!canEdit} onChange={(e) => {
                if (e.target.value) {
              ...
              <div>
                 ...
                 <input type="url" className={inputClass} defaultValue={doc.url || ""} disabled={!canEdit} onBlur={(e) => updateEntity("documents", doc.id, { url: e.target.value })} placeholder="https://..."/>
              </div>
           </div>
        </CenteredBlock>);
    }
    if (node.type === "item" && node.collection === "payments") {
        ...
        return (<CenteredBlock eyebrow="Édition Finance" title={payment.label} onClose={onClose} leading={<Folder className="mt-4 w-6 h-6 text-foreground/50"/>}>
           <div className="space-y-6 mt-4">
              <div>
    ...
    if (node.type === "item" && node.collection === "guests") {
        ...
        return (<CenteredBlock eyebrow="Édition Réseau" title={guest.name} onClose={onClose} leading={<Network className="mt-4 w-6 h-6 text-foreground/50"/>}>
           <div className="space-y-6 mt-4">
              <div>
                 <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Nom</label>
                 <input type="text" className={inputClass} defaultValue={guest.name} disabled={!canEdit} onBlur={(e) => updateEntity("guests", guest.id, { name: e.target.value })}/>
              </div>
              <div className="grid grid-cols-2 gap-4">
                 <div>
                    <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Contact (Email/Tel)</label>
                    <input type="text" className={inputClass} defaultValue={guest.contact || ""} disabled={!canEdit} onBlur={(e) => updateEntity("guests", guest.id, { contact: e.target.value })}/>
                 </div>
                 <div>
                    <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Rôle</label>
                    <select className={selectClass} defaultValue={guest.role} disabled={!canEdit} onChange={(e) => updateEntity("guests", guest.id, { role: e.target.value })}>
              ...
              <div>
                 ...
                 <textarea className={cn(inputClass, "min-h-[80px] resize-none")} defaultValue={guest.notes || ""} disabled={!canEdit} onBlur={(e) => updateEntity("guests", guest.id, { notes: e.target.value })}/>
              </div>
           </div>
        </CenteredBlock>);
    }
    if (node.type === "item" && node.collection === "memories") {
        ...
        return (<CenteredBlock eyebrow="Édition Média" title={memory.title} onClose={onClose} leading={<ImageIcon className="mt-4 w-6 h-6 text-foreground/50"/>}>
           <div className="space-y-6 mt-4">
              <div>
                 <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Titre</label>
                 <input type="text" className={inputClass} defaultValue={memory.title} disabled={!canEdit} onBlur={(e) => updateEntity("memories", memory.id, { title: e.target.value })}/>
              </div>
              <div>
                 <label className="text-[10px] uppercase tracking-widest text-foreground/50 block mb-2">Notes</label>
                 <textarea className={cn(inputClass, "min-h-[100px] resize-none")} defaultValue={memory.notes || ""} disabled={!canEdit} onBlur={(e) => updateEntity("memories", memory.id, { notes: e.target.value })}/>
              </div>
           </div>
        </CenteredBlock>);
    }
    return (<CenteredBlock eyebrow="Détails" title={node.label || "Élément"} onClose={onClose}>
      <p className="mt-4 text-sm text-foreground/60 font-light">Cet élément ne peut pas être modifié dans cette vue.</p>
    </CenteredBlock>);
}
//# sourceMappingURL=EntityEditor.jsx.map