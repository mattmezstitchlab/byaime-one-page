...
export function GuestPanel() {
    ...
    const confirmedCount = useMemo(() => project?.guests.filter(guest => links[guest.id]?.response?.status === "confirmed" || (!links[guest.id]?.response && guest.rsvp === "confirme")).length ?? 0, [links, project?.guests]);
    useEffect(() => {
        ...
        void refreshParticipantLinks()
            .catch(error => {
            if (active)
                setNotice(error instanceof Error ? error.message : "Liens RSVP indisponibles");
        });
        return () => { active = false; };
    ...
    const createParticipantLink = async (guestId) => {
        setBusyGuestId(guestId);
        setNotice("");
        try {
            const link = await api(`/projects/${project.id}/rsvp-links/${guestId}`, { method: "POST" });
    ...
    const copyParticipantLink = async (link) => {
        try {
            await navigator.clipboard.writeText(participantUrl(link.token));
    ...
    const revokeParticipantLink = async (guestId) => {
        setBusyGuestId(guestId);
        setNotice("");
        try {
...
//# sourceMappingURL=GuestPanel.jsx.map