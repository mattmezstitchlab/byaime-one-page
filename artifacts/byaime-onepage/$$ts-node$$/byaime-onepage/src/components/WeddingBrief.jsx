...
import { useGetWeddingBrief, useGetWeddingBriefNearby } from "@workspace/api-client-react";
...
export function WeddingBrief({ project, onOpenMoment, }) {
    ...
    if (briefQuery.isLoading) {
        return (<section aria-labelledby="wedding-brief-loading" className="rounded-[2rem] border border-foreground/10 bg-card p-8 md:p-10">
    ...
    if (briefQuery.isError || !briefQuery.data?.segments.length) {
        return (<section aria-labelledby="wedding-brief-error" className="rounded-[2rem] border border-foreground/10 bg-card p-8 md:p-10">
    ...
    return (<section aria-labelledby="wedding-brief-title" className="overflow-hidden rounded-[2rem] border border-foreground/10 bg-card">
      <div className={cn("grid min-h-[360px]", showNearby && "lg:grid-cols-[1.2fr_.8fr]")}>
          ...
          <div className="relative flex h-full flex-col">
            ...
            <div aria-live="polite" aria-atomic="true">
              <AnimatePresence mode="wait">
                <motion.div key={segment.id} initial={reducedMotion ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={reducedMotion ? undefined : { opacity: 0, y: -12 }} transition={{ duration: reducedMotion ? 0 : .35 }} className="my-auto py-10">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button type="button" aria-label={reducedMotion ? "Séquence suivante" : playing ? "Mettre la lecture automatique en pause" : "Démarrer la lecture automatique"} onClick={() => reducedMotion ? move(1) : setPlaying(value => !value)} className="flex h-11 items-center gap-2 rounded-full bg-foreground px-5 text-[10px] font-semibold uppercase tracking-widest text-background">
        ...
        {showNearby && (<aside className="border-t border-foreground/10 bg-foreground/[.025] p-7 lg:border-l lg:border-t-0 md:p-10">
            <div className="flex h-full flex-col">
              ...
              {brief.location.available ? (<button type="button" onClick={() => {
                    if (locationConsent) {
                ...
                }} aria-pressed={locationConsent} className={cn("mt-6 rounded-xl border px-4 py-3 text-left text-xs transition", locationConsent ? "border-violet-300/30 bg-violet-300/10 text-foreground" : "border-foreground/10 text-foreground/55 hover:bg-foreground/5")}>
                  {locationConsent ? "Lieu autorisé pour cette recherche" : "Utiliser le lieu confirmé de ce Monde"}
                </button>) : (<p className="mt-6 rounded-xl border border-dashed border-foreground/10 p-4 text-xs leading-6 text-foreground/40">Confirmez d’abord une ville ou un lieu dans le Monde.</p>)}
              <div className="mt-6 space-y-2">
                ...
                {nearby?.nearbyCategories.map(item => (<div key={item.id} className="rounded-xl border border-dashed border-foreground/10 p-3">
      ...
      {playing && !reducedMotion && (<motion.div key={segment.id} className="h-1 bg-gradient-to-r from-violet-400 to-sky-300" initial={{ width: 0 }} animate={{ width: "100%" }} transition={{ duration: 6, ease: "linear" }} onAnimationComplete={() => setIndex(value => (value + 1) % brief.segments.length)}/>)}

      {selectedSource && (<ContextPanel eyebrow="Source autorisée" title={selectedSource.source.label} onClose={() => setSelectedSource(undefined)}>
...
function SourceCard({ source }) {
    return (<div className="rounded-xl border border-foreground/10 bg-foreground/[.025] p-4 text-xs">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-foreground/35">{source.collection}</p>
          <p className="mt-1 text-foreground/75">{source.label}</p>
        </div>
        {source.amountCents !== undefined && (<span className="text-right text-foreground/65">
...
//# sourceMappingURL=WeddingBrief.jsx.map