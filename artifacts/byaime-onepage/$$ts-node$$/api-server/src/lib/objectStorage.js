...
import { canAccessObject, getObjectAclPolicy, ObjectPermission, setObjectAclPolicy, } from "./objectAcl";
import { createE2EObjectFile, e2eObjectId, e2eUploadURL, isE2ETestServicesEnabled, } from "./e2eTestServices";
...
function createObjectStorageClient() {
    ...
    if (provider !== "gcs") {
        throw new Error(`Unsupported OBJECT_STORAGE_PROVIDER "${provider}". Supported providers: gcs`);
    }
...
export class ObjectStorageService {
    ...
    getPublicObjectSearchPaths() {
        ...
        if (prefixesStr) {
            const prefixes = Array.from(new Set(prefixesStr
                .split(",")
                .map((path) => path.trim())
...
function objectPathFromRawValue(rawPath) {
    ...
    if (url.hostname.endsWith(".storage.googleapis.com")) {
        const bucketName = url.hostname.slice(0, -".storage.googleapis.com".length);
