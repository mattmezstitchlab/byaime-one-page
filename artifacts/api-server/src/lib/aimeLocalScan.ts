...
export type LocalScanMetadata = {
  name: string;
  extension: string;
  fileType: LocalFileCategory;
...
const EXT_TO_TYPE: Record<string, LocalFileCategory> = {
  pdf: "pdf",
  jpg: "image",
  jpeg: "image",
  png: "image",
...
const DOCUMENT_HINTS: Array<{ type: LocalDocumentType; patterns: RegExp[] }> = [
...
function emptyEntities(): LocalScanEntities {
  return {
    people: [],
    places: [],
    dates: [],
...
export function classifyFileType(fileName: string): LocalFileCategory {
  const extension = path.extname(fileName).replace(/^\./, "").toLowerCase();
  return EXT_TO_TYPE[extension] ?? "other";
}

export function classifyDocumentType(fileName: string, snippet = ""): LocalDocumentType {
...
export function extractLightEntities(fileName: string, snippet = ""): LocalScanEntities {
...
function projectTokens(projectData: Record<string, unknown>): string[] {
  const universe = String(projectData.universe ?? "");
  const title = String(projectData.title ?? "");
  const city = typeof projectData.city === "object" ? String((projectData.city as Record<string, unknown>).value ?? "") : "";
  const venue = typeof projectData.venue === "object" ? String((projectData.venue as Record<string, unknown>).value ?? "") : "";
...
export function suggestForProject(
  file: Pick<LocalScanMetadata, "name" | "documentType" | "entities">,
