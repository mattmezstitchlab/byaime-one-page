...
| « Raconter autrement, en une phrase » | bascule vers une `textarea` libre ; l'envoi devient « Créer mon Monde » / « Continuez la préparation d’AIME » selon la session, et le lien de bascule devient « Revenir aux questions » |
| Envoi | connecté → `createProjectFromIntention` puis `/user-portal` ; sinon brouillon `aime-intention-draft` puis `/creation`, repris à l'hydratation du premier écran |
...
- Aperçu local : `cd artifacts/byaime-onepage && ./node_modules/.bin/vite -c preview/vite.preview.mjs`, puis http://localhost:4173. Le dispositif est **suivi dans le dépôt** (`artifacts/byaime-onepage/preview/`) : un stub de session (`clerk-stub.tsx`), une fausse API en mémoire pour les quatre routes du store (`vite.preview.mjs`) et un contrôle sans navigateur (`preview/smoke.mjs`, `node preview/smoke.mjs`). Aucune de ces pièces n'entre dans le build : `vite.config.ts`, la config de production, n'aliasse rien.
- Sur `/creation` et `/connexion`, l'aperçu affiche une carte expliquant que le formulaire Clerk est simulé, **la phrase retenue sur l'accueil**, et un bouton « Entrer dans l'espace de démonstration » qui simule la session et ouvre `/user-portal` : le parcours accueil → saisie → création de Monde peut donc être parcouru sans clé Clerk ni base de données.
- Recette de l'image de partage : `artifacts/byaime-onepage/preview/og-image.md`.
